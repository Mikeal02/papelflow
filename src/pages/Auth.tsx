...
import { Mail, Lock, User, ArrowRight, TrendingUp, Shield, BarChart3, Wallet, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <motion.div
        ...
        className="hidden lg:flex lg:w-1/2 relative bg-gradient-to-br from-muted via-muted/80 to-background p-12 flex-col justify-between overflow-hidden"
      >
        ...
        <div className="relative z-10">
          ...
          <p className="text-muted-foreground text-sm">Professional finance management</p>
        </div>
        ...
        <div className="relative z-10 space-y-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="space-y-4">
            ...
            <p className="text-lg text-muted-foreground max-w-md">
              Track expenses, set budgets, and achieve goals with powerful analytics.
            </p>
          </motion.div>
          ...
          <div className="space-y-3">
            {features.map((feature, index) => (
              <motion.div
                ...
                whileHover={{ x: 4 }}
                className="flex items-start gap-3 p-4 rounded-xl bg-card/60 border border-border/50 backdrop-blur-sm"
              >
        ...
        <div className="relative z-10 text-xs text-muted-foreground">© 2026 Finflow. All rights reserved.</div>
      </motion.div>
      ...
      >
        <div className="w-full max-w-md space-y-6">
          ...
          <div className="text-center lg:text-left space-y-2">
            ...
            <p className="text-muted-foreground text-sm">{isLogin ? 'Sign in to access your dashboard' : 'Start your journey to financial freedom'}</p>
          </div>
          ...
          <div className="text-center">
            <button onClick={() => setIsLogin(!isLogin)} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
              {isLogin ? <>Don't have an account? <span className="text-primary font-semibold">Sign up</span></> : <>Already have an account? <span className="text-primary font-semibold">Sign in</span></>}
            </button>
          </div>
