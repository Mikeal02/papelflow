...
export default function RecurringPayments() {
      ...
      while (isBefore(rangeStart, checkDate)) {
        const prev =
          sub.frequency === "weekly"
            ? subMonths(checkDate, 0)
            : subMonths(checkDate, sub.frequency === "monthly" ? 1 : 12);
  ...
  const monthPayments = calendarData.filter((p) =>
    isSameMonth(p.date, currentMonth),
  );
  const filteredPayments =
    filterStatus === "all"
      ? monthPayments
      : monthPayments.filter((p) => p.status === filterStatus);
  ...
  const stats = useMemo(() => {
    ...
    const upcoming = mp.filter(
      (p) => p.status === "upcoming" || p.status === "due-today",
    );
  ...
  const getStatusIcon = (status: PaymentStatus) => {
    switch (status) {
      case "paid":
        return <CheckCircle2 className="h-3.5 w-3.5 text-income" />;
      case "overdue":
        return <XCircle className="h-3.5 w-3.5 text-expense" />;
      case "due-today":
  ...
  const getStatusBadge = (status: PaymentStatus) => {
    const map = {
      ...
      overdue: {
        label: "Overdue",
        className: "bg-expense/10 text-expense border-expense/20",
      },
      "due-today": {
        label: "Due Today",
    ...
    return (
      <Badge
        variant="outline"
        className={cn("text-[10px] px-1.5 py-0", s.className)}
      >
        {s.label}
      </Badge>
    );
  ...
  const dayPayments = (day: Date) =>
    calendarData.filter((p) => isSameDay(p.date, day));
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Track scheduled payments, auto-pay, and history
              </p>
            </div>
          </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-mono text-muted-foreground">
                {stats.paidPercent.toFixed(0)}% complete
              </span>
            </div>
            ...
            <div className="flex justify-between mt-2 text-xs text-muted-foreground">
              ...
              <span>
                {formatCurrency(stats.totalDue - stats.totalPaid)} remaining
              </span>
            </div>
          </CardContent>
        </Card>
              ...
              <CardContent className="p-4">
                ...
                <div className="grid grid-cols-7 gap-1">
                  {calendarDays.map((day) => {
                    ...
                    const hasDueToday = payments.some(
                      (p) => p.status === "due-today",
                    );
                ...
                >
                  <Card className="stat-card">
                    <CardHeader className="pb-3">
                      ...
                      <CardDescription>
                        {dayPayments(selectedDay).length} payment(s)
                      </CardDescription>
                    </CardHeader>
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                  ...
                                  {p.isAutoPay && (
                                    <Badge
                                      variant="outline"
                                      className="text-[9px] px-1 py-0 text-accent border-accent/20"
                                    >
                          ...
                          <div className="text-right shrink-0">
                            ...
                            {payment.status === "overdue" && (
                              <p className="text-[10px] text-expense">
                                {differenceInDays(today, payment.date)}d overdue
                              </p>
                            )}
                            {payment.status === "upcoming" && (
                              <p className="text-[10px] text-muted-foreground">
                                in {differenceInDays(payment.date, today)}d
                              </p>
                            )}
                          </div>
                        </div>
          ...
          <TabsContent value="history" className="space-y-4">
            <Card className="stat-card">
              <CardHeader>
                ...
                <CardDescription>
                  Recent recurring payment activity
                </CardDescription>
              </CardHeader>
              <CardContent>
                {paymentHistory.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-muted-foreground">
                      No recurring payment history yet
                    </p>
                  </div>
                ) : (
                  <div className="relative">
                    ...
                    <div className="space-y-1">
                      {paymentHistory.map((tx, i) => {
                        ...
                        const prevDate =
                          i > 0 ? paymentHistory[i - 1].date : null;
