...
const TAX_BRACKETS_SINGLE = [
  { min: 0, max: 11925, rate: 0.1, label: "10%" },
...
const TAX_BRACKETS_MARRIED = [
  { min: 0, max: 23850, rate: 0.1, label: "10%" },
...
export default function TaxEstimator() {
  ...
  const effectiveRate =
    projectedGross > 0 ? (projectedTax.total / projectedGross) * 100 : 0;
  const marginalRate =
    projectedTax.breakdown.length > 0
      ? projectedTax.breakdown[projectedTax.breakdown.length - 1].rate * 100
      : 0;
  ...
  const deductionPieData = deductions
    .filter((d) => d.amount > 0)
    .map((d) => ({
      name: d.name,
      value: d.amount,
  ...
  const taxBreakdownPie = [
    ...
    },
    {
      ...
      value: Math.max(
        0,
        projectedGross - projectedTax.total - effectiveDeduction,
      ),
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Estimate your federal tax liability with quarterly projections
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Select
              value={filingStatus}
              onValueChange={(v: "single" | "married") => setFilingStatus(v)}
            >
        ...
        <div className="grid gap-3 grid-cols-2 lg:grid-cols-5">
          {[
            ...
            },
            {
              ...
              value: formatCurrency(projectedTax.total),
              icon: Receipt,
              color: "text-expense",
            },
            {
            ...
            {
              label: owedOrRefund >= 0 ? "Estimated Owed" : "Estimated Refund",
              value: formatCurrency(Math.abs(owedOrRefund)),
              icon: owedOrRefund >= 0 ? AlertTriangle : CheckCircle2,
              color: owedOrRefund >= 0 ? "text-expense" : "text-income",
            },
          ].map((kpi, i) => (
            <motion.div
              key={kpi.label}
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    How your projected income breaks down
                  </CardDescription>
                </CardHeader>
                <CardContent>
                      ...
                      >
                        ...
                        >
                          {taxBreakdownPie.map((entry, i) => (
                            <Cell key={`income-pie-${i}`} fill={entry.fill} />
                          ))}
                        </Pie>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Projected annual tax calculation
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  {[
                    ...
                    },
                    {
                      label: useItemized
                        ? "Itemized Deductions"
                        : "Standard Deduction",
                      value: `(${formatCurrency(effectiveDeduction)})`,
                      bold: false,
                    },
                    {
                      label: "Taxable Income",
                      value: formatCurrency(
                        Math.max(0, projectedGross - effectiveDeduction),
                      ),
                      bold: true,
                    },
                    {
                    ...
                    {
                      label: "Tax Withheld (YTD)",
                      value: `(${formatCurrency(withheld)})`,
                      bold: false,
                    },
                    {
                      label:
                        owedOrRefund >= 0 ? "Balance Due" : "Refund Expected",
                      value: formatCurrency(Math.abs(owedOrRefund)),
                      bold: true,
                    },
                  ].map((row, i) => (
                    <div
                      ...
                      className={cn(
                        ...
                        row.bold && "border-t border-border pt-2",
                      )}
                    >
                      <span
                        className={cn(
                  ...
                  <div className="mt-4 space-y-2">
                    <Label className="text-xs text-muted-foreground">
                      Additional Income (Side jobs, etc.)
                    </Label>
                    <Input
                      type="number"
          ...
          <TabsContent value="brackets" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  See how your income is taxed across progressive brackets (
                  {filingStatus === "single"
                    ? "Single"
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {projectedTax.breakdown.map((b, i) => {
                        const cumulative = projectedTax.breakdown
                          .slice(0, i + 1)
                          .reduce((s, x) => s + x.tax, 0);
                        return (
                          <tr key={i} className="border-b border-border/50">
                            ...
                            <td className="py-2.5 text-right font-mono text-foreground">
                              {formatCurrency(b.taxed)}
                            </td>
                            <td className="py-2.5 text-right font-mono text-expense">
                    ...
                    <tfoot>
                      <tr className="border-t-2 border-border font-bold">
                        ...
                        <td className="py-2.5 text-right font-mono text-expense">
                          {formatCurrency(projectedTax.total)}
                        </td>
                        <td></td>
                      </tr>
                    </tfoot>
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <CardDescription>
                          Track itemized deductions vs standard deduction
                        </CardDescription>
                      </div>
                      <div className="flex items-center gap-2">
                        <Button
                          variant={useItemized ? "default" : "outline"}
                          size="sm"
                    ...
                    <div className="rounded-lg bg-muted/50 p-4 space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">
                          Standard: {formatCurrency(standardDed)}
                        </span>
                        <span className="text-muted-foreground">
                          Itemized: {formatCurrency(itemizedTotal)}
                        </span>
                      </div>
                      <div className="relative h-3 rounded-full bg-muted overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, (standardDed / Math.max(standardDed, itemizedTotal)) * 100)}%`,
                          }}
                        />
                        <div
                          className="absolute inset-y-0 left-0 bg-accent rounded-full"
                          style={{
                            width: `${Math.min(100, (itemizedTotal / Math.max(standardDed, itemizedTotal)) * 100)}%`,
                            opacity: 0.6,
                          }}
                        />
                      </div>
                      {itemizedTotal > standardDed ? (
                        <p className="text-xs text-income flex items-center gap-1">
                          <CheckCircle2 className="h-3 w-3" /> Itemizing saves
                          you {formatCurrency(itemizedTotal - standardDed)}
                        </p>
                      ) : (
                        <p className="text-xs text-muted-foreground flex items-center gap-1">
                          <Info className="h-3 w-3" /> Standard deduction is
                          better by{" "}
                          {formatCurrency(standardDed - itemizedTotal)}
                        </p>
                      )}
                    </div>
                      ...
                      {deductions.map((d) => (
                        <div key={d.id} className="flex items-center gap-2">
                          ...
                          <Select
                            value={d.category}
                            onValueChange={(v) =>
                              updateDeduction(d.id, "category", v)
                            }
                          >
          ...
          <TabsContent value="quarterly" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Track income and estimated payments by quarter
                </CardDescription>
              </CardHeader>
              <CardContent>
              ...
              <CardContent className="p-4 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    To avoid underpayment penalties, pay at least 90% of your
                    current year tax liability or 100% of your prior year tax
                    (110% if AGI &gt; $150,000) through withholding and
                    estimated payments.
                  </p>
                </div>
              </CardContent>
