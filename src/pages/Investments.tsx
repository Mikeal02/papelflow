...
function generateDividendHistory(): Dividend[] {
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    divs.push({
      ...
      type: Math.random() > 0.3 ? "qualified" : "ordinary",
    });
  }
  return divs;
}
...
export default function Investments() {
  ...
  const analytics = useMemo(() => {
    ...
    const annualDividends = holdings.reduce(
      (s, h) => s + (h.shares * h.currentPrice * h.dividendYield) / 100,
      0,
    );
    const portfolioYield =
      totalValue > 0 ? (annualDividends / totalValue) * 100 : 0;
    ...
    const allocation = ASSET_CLASSES.map((ac) => ({
      ...
      percentage:
        totalValue > 0 ? ((allocationMap[ac.key] || 0) / totalValue) * 100 : 0,
      color: ac.color,
    })).filter((a) => a.value > 0);
    ...
    // Top movers
    const holdingsWithGain = holdings
      .map((h) => ({
        ...h,
        marketValue: h.shares * h.currentPrice,
        gain: (h.currentPrice - h.avgCost) * h.shares,
        returnPct:
          h.avgCost > 0 ? ((h.currentPrice - h.avgCost) / h.avgCost) * 100 : 0,
      }))
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Track asset allocation, performance, and dividends
              </p>
            </div>
          </div>
            ...
            <Button
              ...
              onClick={() => {
                generatePortfolioStatementPDF(
                  ...
                  analytics.totalCost,
                  analytics.totalGain,
                  analytics.totalReturn,
                  analytics.annualDividends,
                  analytics.portfolioYield,
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          setNewHolding({ ...newHolding, assetClass: v })
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                        ...
                        <SelectContent>
                          {ASSET_CLASSES.map((ac) => (
                            <SelectItem key={ac.key} value={ac.key}>
                              {ac.label}
                            </SelectItem>
                          ))}
                        </SelectContent>
        ...
        <div className="grid gap-3 grid-cols-2 lg:grid-cols-6">
          {[
            ...
            },
            {
              ...
              icon: analytics.totalGain >= 0 ? TrendingUp : TrendingDown,
              color: analytics.totalGain >= 0 ? "text-income" : "text-expense",
            },
            {
              label: "Total Return",
              value: `${analytics.totalReturn.toFixed(1)}%`,
              icon: Percent,
              color:
                analytics.totalReturn >= 0 ? "text-income" : "text-expense",
            },
            {
              label: "Annual Dividends",
              value: formatCurrency(analytics.annualDividends),
              icon: Coins,
              color: "text-accent",
            },
            {
              label: "Portfolio Yield",
              value: `${analytics.portfolioYield.toFixed(2)}%`,
              icon: Target,
              color: "text-warning",
            },
          ].map((kpi, i) => (
        ...
        <Tabs defaultValue="performance" className="space-y-4">
          ...
          <TabsContent value="performance" className="space-y-4">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base text-income">
                    Top Gainers
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-2">
                  {analytics.holdingsWithGain
                    ...
                    .map((h) => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          <Badge
                            variant="secondary"
                            className="text-income text-xs font-mono"
                          >
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base text-expense">
                    Underperformers
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-2">
                    ...
                    .map((h) => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          <Badge
                            variant="secondary"
                            className="text-expense text-xs font-mono"
                          >
                  ...
                  {analytics.holdingsWithGain.filter((h) => h.returnPct <= 0)
                    .length === 0 && (
          ...
          <TabsContent value="holdings" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {holdings.length} positions across{" "}
                  {analytics.allocation.length} asset classes
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border">
                        {[
                          ...
                          <th
                            key={h}
                            className="text-left py-2 text-muted-foreground font-medium text-xs whitespace-nowrap px-1"
                          >
                            {h}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {analytics.holdingsWithGain.map((h) => (
                        ...
                        >
                          ...
                          <td className="py-2.5 px-1 text-foreground max-w-[150px] truncate">
                            {h.name}
                          </td>
                          <td className="py-2.5 px-1 font-mono text-foreground">
                          ...
                          <td className="py-2.5 px-1 font-mono font-medium text-foreground">
                            {formatCurrency(h.marketValue)}
                          </td>
                          <td
                            className={cn(
                              "py-2.5 px-1 font-mono",
                              h.gain >= 0 ? "text-income" : "text-expense",
                            )}
                          >
                            {h.gain >= 0 ? "+" : ""}
                            {formatCurrency(h.gain)}
                    ...
                    <tfoot>
                      <tr className="border-t-2 border-border font-bold">
                        <td className="py-2.5 px-1 text-foreground" colSpan={5}>
                          Total
                        </td>
                        <td className="py-2.5 px-1 font-mono text-foreground">
                        ...
                        <td className="py-2.5 px-1 font-mono text-accent">
                          {analytics.portfolioYield.toFixed(2)}%
                        </td>
                        <td></td>
                      </tr>
                    </tfoot>
              ...
              <div className="lg:col-span-2">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Trailing 12-month dividend payments
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="h-[300px]">
                  ...
                  <CardContent className="space-y-3">
                    {[
                      ...
                      },
                      {
                        ...
                        value: formatCurrency(analytics.annualDividends / 12),
                      },
                      {
                        label: "YTD Received",
                        value: formatCurrency(analytics.totalDividendsYTD),
                      },
                      {
                        label: "Portfolio Yield",
                        value: `${analytics.portfolioYield.toFixed(2)}%`,
