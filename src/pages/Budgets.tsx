...
const Budgets = () => {
  ...
  const categorySpending = useMemo(() => {
    ...
    transactions
      .filter((t) => {
        ...
        return (
          t.type === "expense" && txDate >= monthStart && txDate <= monthEnd
        );
      })
      .forEach((t) => {
        if (t.category_id)
  ...
  const needsBudgets = monthBudgets.filter(
    (b) => b.category?.category_group === "Needs",
  );
  const wantsBudgets = monthBudgets.filter(
    (b) => b.category?.category_group === "Wants",
  );
  const otherBudgets = monthBudgets.filter(
    (b) =>
      b.category?.category_group !== "Needs" &&
      b.category?.category_group !== "Wants",
  );
  ...
  const nearLimitCount = monthBudgets.filter(
    (b) => b.percentage >= 80 && b.percentage <= 100,
  ).length;

  const pieData = monthBudgets
    .map((b) => ({
      name: b.category?.name || "",
      value: b.spent,
      fill: b.category?.color || "#6366F1",
    }))
    .filter((d) => d.value > 0);
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1.5">
              <p className="text-[10px] text-muted-foreground">
                {budget.rollover ? "Rollover" : "Fixed"}
              </p>
              {budget.change !== 0 && budget.prevSpent > 0 && (
                ...
                >
                  {budget.change > 0 ? "↑" : "↓"}
                  {Math.abs(budget.change).toFixed(0)}%
                </Badge>
              )}
            </div>
          </div>
      ...
      <div className="space-y-2">
        ...
        <div className="relative h-2 overflow-hidden rounded-full bg-muted/50">
          <motion.div
            ...
            transition={{ delay: delay + 0.05 + index * 0.05, duration: 0.6 }}
            className={cn(
        ...
        >
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              Monitor spending limits and category budgets
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1 bg-muted/50 rounded-xl p-1 border border-border/30">
              ...
              <span className="px-3 font-semibold text-sm min-w-[120px] text-center">
          ...
          <div className="stagger-kinetic grid gap-5 grid-cols-2 lg:grid-cols-4">
            {[
              ...
              },
              {
                ...
                icon: TrendingDown,
                color:
                  budgetProgress > 100 ? "text-expense" : "text-foreground",
              },
              {
                label: "On Budget",
                value: `${onBudgetCount}/${monthBudgets.length}`,
                icon: CheckCircle2,
                color: "text-income",
                sub: `${nearLimitCount} near limit`,
              },
              {
                label: "Over Budget",
              ...
              <div className="flex-1 max-w-md">
                <div className="flex justify-between text-sm mb-2">
                  ...
                  >
                    ...
                    {totalBudget - totalSpent < 0 ? "over" : "remaining"}
                  </span>
                </div>
          ...
          >
            <Tabs defaultValue="breakdown" className="stat-card">
              <TabsList className="stagger-kinetic grid w-full grid-cols-2 mb-4">
                ...
                <TabsTrigger value="vs-budget" className="text-xs gap-1.5">
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      data={monthBudgets.map((b) => ({
                        name: (b.category?.name || "").slice(0, 8),
                ...
                >
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <Badge variant="secondary" className="text-[10px]">
                      Essential • {needsBudgets.length} categories
                    </Badge>
                  </div>
              ...
              {wantsBudgets.length > 0 && (
                ...
                >
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <Badge variant="secondary" className="text-[10px]">
                      Discretionary • {wantsBudgets.length} categories
                    </Badge>
                  </div>
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              {otherBudgets.length > 0 && (
                ...
                >
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <Badge variant="secondary" className="text-[10px]">
                      {otherBudgets.length} categories
                    </Badge>
                  </div>
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {otherBudgets.map((budget, index) => (
                      <BudgetCard
                        key={budget.id}
                        budget={budget}
                        index={index}
                ...
                >
                  <div className="flex items-center gap-2.5 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        {unbudgetedSpending.length} categories without budgets
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    {unbudgetedSpending.slice(0, 5).map((item, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between gap-2 p-3 rounded-2xl bg-muted/20 border border-border/30 backdrop-blur-sm hover:bg-muted/30 transition-all duration-200"
                      >
                        ...
                        <span className="text-sm font-bold text-expense shrink-0">
