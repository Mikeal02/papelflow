...
const NetWorth = () => {
  ...
  const { assets, liabilities, totalAssets, totalLiabilities, netWorth } =
    useMemo(() => {
      ...
      const totalAsset = assetAccounts.reduce(
        (sum, a) => sum + Math.max(0, Number(a.balance || 0)),
        0,
      );
  ...
  const assetAllocation = useMemo(() => {
    return assets
      .map((a) => ({
        name: a.name,
        value: Math.max(0, Number(a.balance || 0)),
      }))
      .filter((a) => a.value > 0);
  }, [assets]);
  ...
  const netWorthHistory = useMemo(() => {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      result.push({
        ...
        netWorth: Math.max(
          0,
          netWorth + runningBalance - (income - expenses) * (12 - i) * 0.08,
        ),
      });
    }
    ...
    if (result.length > 0) {
      ...
      result.forEach((r, i) => {
        r.netWorth = Math.max(0, r.netWorth + diff * (i / result.length));
      });
  ...
  const percentChange =
    previousMonth > 0
      ? ((monthlyChange / previousMonth) * 100).toFixed(1)
      : "0";
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="grid grid-cols-3 gap-4 md:gap-6">
              <div className="text-center">
                ...
                <p className="text-sm md:text-base font-bold text-income truncate">
              ...
              <div className="text-center">
                ...
                <p className="text-sm md:text-base font-bold text-expense truncate">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground truncate">
                  {formatCurrency(totalAssets)} • {assets.length} accounts
                </p>
              </div>
            </div>
            {assets.length > 0 ? (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <Badge
                            variant="outline"
                            className="text-[9px] capitalize shrink-0 hidden sm:inline-flex"
                          >
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground truncate">
                  {formatCurrency(totalLiabilities)} • {liabilities.length}{" "}
                  accounts
                </p>
              </div>
            </div>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          <div
                            className="h-7 w-7 rounded-lg flex items-center justify-center shrink-0"
                            style={{
                              backgroundColor: `${account.color || "#EF4444"}20`,
                            }}
                          >
                            <CreditCard
                              className="h-3.5 w-3.5"
                        ...
                        <span className="font-semibold text-expense text-sm shrink-0">
                          {formatCurrency(
                            Math.abs(Number(account.balance || 0)),
                          )}
                        </span>
