...
const DebtTracker = () => {
  ...
  const avgRate =
    debtAccounts.length > 0
      ? debtAccounts.reduce((s, a) => s + a.interestRate * a.owed, 0) /
        (totalDebt || 1)
      : 0;
  const highestRate =
    debtAccounts.length > 0
      ? Math.max(...debtAccounts.map((a) => a.interestRate))
      : 0;
  ...
  const monthsToPayoff = useMemo(() => {
    ...
    const months =
      -Math.log(1 - (totalDebt * monthlyRate) / monthlyPayment) /
      Math.log(1 + monthlyRate);
    return Math.ceil(
      isFinite(months) && months > 0 ? months : totalDebt / monthlyPayment,
    );
  }, [totalDebt, monthlyPayment, avgRate]);

  const totalInterest = useMemo(
    () => Math.max(0, monthsToPayoff * monthlyPayment - totalDebt),
  ...
  const payoffProjection = useMemo(() => {
    ...
    for (
      let i = 0;
      i <= Math.min(monthsToPayoff, 60);
      i += Math.max(1, Math.floor(monthsToPayoff / 12))
    ) {
          ...
          {[
            {
              ...
              sub: `${debtAccounts.length} accounts`,
              color: "expense",
            },
            {
              icon: DollarSign,
            ...
            },
            {
              ...
              sub: `~${(monthsToPayoff / 12).toFixed(1)} years`,
              color: "primary",
            },
            {
              icon: AlertTriangle,
              ...
              sub: `${avgRate.toFixed(1)}% avg / ${highestRate}% max`,
              color: "expense",
            },
          ].map((item, i) => (
            <motion.div
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <div
                  className={`flex h-8 w-8 items-center justify-center rounded-lg bg-${item.color}/10 shrink-0`}
                >
              ...
              <div className="grid grid-cols-3 gap-3 pt-2 border-t border-border/30">
                ...
                <div className="text-center">
                  ...
                  <p className="text-lg font-bold">
                    {formatCurrency(monthsToPayoff * monthlyPayment)}
                  </p>
                </div>
              </div>
            </div>
              ...
              {sortedDebts.length === 0 ? (
                <div className="stat-card flex flex-col items-center py-16">
                  ...
                  <p className="text-sm text-muted-foreground mb-4 text-center">
                    Add credit card or loan accounts to start tracking debt
                  </p>
                </div>
              ) : (
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground">
                              ...
                              Interest:{" "}
                              {formatCurrency(
                                (debt.owed * debt.interestRate) / 100 / 12,
                              )}
                              /mo
                            </p>
                          </div>
