...
const GoalChartTooltip = ({ active, payload, label, formatCurrency }: any) => {
  if (!active || !payload || payload.length === 0) return null;

  return (
    <div
      className="rounded-xl border border-border bg-card px-3 py-2.5 shadow-lg"
      style={{
        fontSize: "12px",
      }}
    >
        ...
        >
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-medium" style={{ color: entry.color }}>
              {entry.name === "saved" ? "Saved" : "Remaining"}
            </span>
          </div>
...
const Goals = () => {
  ...
  const processedGoals = useMemo(() => {
    let filtered = goals.map((g) => {
      ...
      const daysRemaining = g.deadline
        ...
        : null;
      const monthsRemaining = g.deadline
        ? differenceInMonths(new Date(g.deadline), new Date())
        : null;
      const monthlyNeeded =
    ...
    if (sortBy === "deadline")
      ...
      filtered.sort(
        (a, b) => Number(b.target_amount) - Number(a.target_amount),
      );
  ...
  const barData = goals.map((g, i) => ({
    ...
    remaining: Math.max(
      0,
      Number(g.target_amount) - Number(g.current_amount || 0),
    ),
  }));
        ...
        >
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              Track, analyze, and achieve your savings targets
            </p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Button
              className="gap-2 btn-premium h-9"
            ...
            {[
              {
                ...
                sub: `of ${formatCurrency(totalTarget)}`,
                icon: Target,
                color: "text-income",
              },
              {
              ...
              },
              {
                ...
                sub: `${goals.length > 0 ? ((completedCount / goals.length) * 100).toFixed(0) : 0}% success`,
                icon: Trophy,
                color: "text-accent",
              },
              {
                label: "Avg. Progress",
                value: `${goals.length > 0 ? (goals.reduce((s, g) => s + (Number(g.current_amount || 0) / Number(g.target_amount)) * 100, 0) / goals.length).toFixed(0) : 0}%`,
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="space-y-1 min-w-0">
                ...
                <div className="flex flex-wrap items-baseline gap-2">
                  <span className="text-2xl sm:text-3xl font-bold text-income">
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={barData} layout="vertical">
                      ...
                      <Tooltip
                        formatter={(value: number) => formatCurrency(value)}
                        content={
                          <GoalChartTooltip formatCurrency={formatCurrency} />
                        }
                        itemStyle={{
                          color: "hsl(var(--foreground))",
                        }}
          ...
          <AnimatePresence mode="popLayout">
            {processedGoals.map((goal, index) => (
              <motion.div
                ...
                className={cn(
                  "stat-card group cursor-default flex flex-col",
                  goal.isCompleted && "ring-1 ring-income/30",
                  goal.isOverdue && "ring-1 ring-expense/30",
                )}
              >
                <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        {goal.isCompleted && (
                          <Badge
                            variant="outline"
                            className="text-[10px] h-5 border-income/30 text-income bg-income/10"
                          >
                            Completed
                          </Badge>
                        )}
                        {goal.isOverdue && (
                          <Badge
                            variant="outline"
                            className="text-[10px] h-5 border-expense/30 text-expense bg-expense/10"
                          >
                            Overdue
                          </Badge>
                        )}
                        {goal.urgency === "urgent" && !goal.isCompleted && (
                          <Badge
                            variant="outline"
                            className="text-[10px] h-5 border-warning/30 text-warning bg-warning/10"
                          >
                            Urgent
                          </Badge>
                        )}
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-40">
                      <DropdownMenuItem
                        onClick={() => {
                          const a = prompt("How much to add?");
                          if (a)
                            updateGoal.mutate({
                              id: goal.id,
                              current_amount:
                                Number(goal.current_amount || 0) + Number(a),
                            });
                        }}
                      >
                        Add funds
                      ...
                      <DropdownMenuItem
                        onClick={() => {
                          const a = prompt("How much to withdraw?");
                          if (a)
                            updateGoal.mutate({
                              id: goal.id,
                ...
                <div className="space-y-2 mt-auto">
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="font-bold text-income truncate">
                ...
                <div className="mt-3 pt-3 border-t border-border/30 grid grid-cols-2 gap-2">
                  ...
                  {goal.weeklyNeeded && goal.weeklyNeeded > 0 && (
                    <div className="p-2.5 rounded-xl bg-muted/20 border border-border/20 backdrop-blur-sm text-center">
                      ...
                      <p className="text-xs font-bold truncate">
                        {formatCurrency(goal.weeklyNeeded)}
                      </p>
                    </div>
                  )}
                  {(!goal.monthlyNeeded || goal.monthlyNeeded <= 0) && (
