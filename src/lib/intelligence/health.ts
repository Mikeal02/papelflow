...
export function computeHealthScore(
  ...
  const last30 = txs.filter(
    (t) => differenceInDays(today, parseISO(t.date)) <= 30,
  );
  const last90 = txs.filter(
    (t) => differenceInDays(today, parseISO(t.date)) <= 90,
  );

  const income30 = sum(
    last30.filter((t) => t.type === "income").map((t) => Number(t.amount)),
  );
  const expense30 = sum(
    last30.filter((t) => t.type === "expense").map((t) => Number(t.amount)),
  );
  ...
  const liquidBalance = sum(
    accounts
      .filter((a) => ["cash", "bank", "wallet"].includes(a.type))
  ...
  const dims = {
    ...
    emergencyBuffer: { score: dBuffer, value: months, weight: 0.2 },
    debtBurden: { score: dDebt, value: debtRatio, weight: 0.15 },
    goalProgress: { score: goalScore, value: goalScore, weight: 0.1 },
    spendingDiscipline: { score: dDiscipline, value: slope, weight: 0.15 },
  };

  const composite = Object.values(dims).reduce(
  ...
  const pi = sum(
    prior30.filter((t) => t.type === "income").map((t) => Number(t.amount)),
  );
  const pe = sum(
    prior30.filter((t) => t.type === "expense").map((t) => Number(t.amount)),
  );
  ...
  const recMap: Record<string, string> = {
    ...
    cashflowStability:
      "Smooth cashflow with a sinking fund for irregular bills.",
  ...
  const sortedDims = Object.entries(dims).sort(
  ...
  const evidence = (() => {
    if (
      weakKey === "savingsRate" ||
      weakKey === "spendingDiscipline" ||
      weakKey === "cashflowStability"
    ) {
  ...
  const explanation: AlgorithmExplanation = {
    ...
    method:
      "Weighted multi-dimensional scoring (savings, stability, buffer, debt, goals, discipline)",
    ...
    diagnostics: [
      ...
      {
        label: "Δ savings rate vs prior month",
  ...
  return {
    ...
    trajectory,
    topRecommendation:
      recMap[weakest[0]] || "Keep going — your habits are solid.",
    explanation,
  };
}
...
function dimensionDescription(k: string, v: number): string {
  switch (k) {
    case "savingsRate":
      return `${v.toFixed(1)}% of income saved (target ≥20%).`;
    case "cashflowStability":
      return `Coefficient of variation ${v.toFixed(2)} (lower = steadier).`;
    case "emergencyBuffer":
      return `${v.toFixed(1)} months of expenses in liquid accounts (target ≥6).`;
    case "debtBurden":
      return `Debt is ${v.toFixed(2)}× liquid balance (target ≤0.5).`;
    case "goalProgress":
      return `Average goal completion ${v.toFixed(0)}%.`;
    case "spendingDiscipline":
      return `Daily expense slope ${v.toFixed(2)} (negative = improving).`;
    default:
      return "";
  }
}
