...
function runSimulation(p: ScenarioInputs): {
  ...
  for (let it = 0; it < p.iterations; it++) {
    ...
    for (let m = 1; m <= p.horizonMonths; m++) {
      ...
      const incThisMonth =
        inc *
        Math.exp(p.incomeShockSigma * gauss() - p.incomeShockSigma ** 2 / 2);
      const expThisMonth =
        exp *
        Math.exp(p.expenseShockSigma * gauss() - p.expenseShockSigma ** 2 / 2);
...
function summarize(
...
): Omit<ScenarioReport, "stressTests" | "durationMs"> {
  ...
  const successProbability =
    sim.finals.filter((v) => v >= p.wealthTarget).length / N;
  ...
  const yearsToTarget = sim.monthsToTarget.length
    ? percentile(
        [...sim.monthsToTarget].sort((a, b) => a - b),
        0.5,
      ) / 12
    : null;
  ...
  const variance =
    cagrs.reduce((a, b) => a + (b - meanCagr) ** 2, 0) /
    Math.max(1, cagrs.length - 1);
...
const STRESS_SCENARIOS: {
  name: string;
  description: string;
  mutate: (p: ScenarioInputs) => ScenarioInputs;
}[] = [
  {
    name: "Recession",
    description:
      "-30% returns, +200bps inflation, doubled swan probability for 2y",
    mutate: (p) => ({
      ...p,
      expectedReturnAnnual: p.expectedReturnAnnual - 0.05,
      inflationAnnual: p.inflationAnnual + 0.02,
      blackSwanAnnualProb: p.blackSwanAnnualProb * 2,
    }),
  },
  {
    name: "Job Loss",
    description: "50% income for 12 months (modeled as halved monthly income)",
    mutate: (p) => ({
      ...p,
      monthlyIncome: p.monthlyIncome * 0.5,
      monthlyContribution: p.monthlyContribution * 0.5,
    }),
  },
  {
    name: "Stagflation",
    description: "Inflation +400bps, returns -300bps",
    mutate: (p) => ({
      ...p,
      inflationAnnual: p.inflationAnnual + 0.04,
      expectedReturnAnnual: p.expectedReturnAnnual - 0.03,
    }),
  },
  {
    name: "Bull Market",
    description: "Returns +400bps, lower vol, low swan risk",
    mutate: (p) => ({
      ...p,
      expectedReturnAnnual: p.expectedReturnAnnual + 0.04,
      returnVolatilityAnnual: Math.max(0.05, p.returnVolatilityAnnual - 0.03),
      blackSwanAnnualProb: p.blackSwanAnnualProb * 0.5,
    }),
  },
  {
    name: "Lifestyle Inflation",
    description: "+25% expenses, contributions halved",
    mutate: (p) => ({
      ...p,
      monthlyExpenses: p.monthlyExpenses * 1.25,
      monthlyContribution: p.monthlyContribution * 0.5,
    }),
  },
  {
    name: "Aggressive Saver",
    description: "Contributions +50%, expenses -10%",
    mutate: (p) => ({
      ...p,
      monthlyContribution: p.monthlyContribution * 1.5,
      monthlyExpenses: p.monthlyExpenses * 0.9,
    }),
  },
];
...
export function runScenarioLab(inputs: ScenarioInputs): ScenarioReport {
  ...
  const stressTests: ScenarioStressTest[] = STRESS_SCENARIOS.map((s) => {
    ...
    const successProb =
      sim.finals.filter((v) => v >= inputs.wealthTarget).length /
      sim.finals.length;
    return {
      name: s.name,
...
function quickRun(
  ...
  iterations = 600,
): { endpointP50: number; successProb: number } {
...
export function runSensitivityAnalysis(
  ...
  const results: SensitivityResult[] = SENSITIVITY_PARAMS.map((p) => {
    ...
    const lo = quickRun(
      { ...inputs, [p.key]: lowValue } as ScenarioInputs,
      iterations,
    );
    const hi = quickRun(
      { ...inputs, [p.key]: highValue } as ScenarioInputs,
      iterations,
    );
    const elasticity =
      base.endpointP50 > 0
    ...
    return {
      ...
      lowValue,
      highValue,
      lowEndpointP50: lo.endpointP50,
      highEndpointP50: hi.endpointP50,
      lowSuccess: lo.successProb,
      highSuccess: hi.successProb,
      elasticity,
      swing: Math.abs(hi.endpointP50 - lo.endpointP50),
    };
  });
  results.sort((a, b) => b.swing - a.swing);
  return {
    base: {
      endpointP50: base.endpointP50,
      successProbability: base.successProb,
    },
    results,
    durationMs: Math.round(performance.now() - t0),
  };
...
export function optimizeContribution(
  ...
  let lo = 0;
  let hi = Math.max(
    inputs.monthlyIncome * 0.8,
    inputs.monthlyContribution * 5,
    1000,
  );
  const trace: { contribution: number; success: number }[] = [];
  const high = quickRun(
    { ...inputs, monthlyContribution: hi },
    searchIterations,
  );
  ...
  if (high.successProb < targetSuccess) {
    return {
      ...
      successAtRequired: high.successProb,
      delta: hi - inputs.monthlyContribution,
  ...
  let best = hi,
    bestSuccess = high.successProb;
  for (let step = 0; step < maxSteps; step++) {
    const mid = (lo + hi) / 2;
    const r = quickRun(
      { ...inputs, monthlyContribution: mid },
      searchIterations,
    );
    trace.push({ contribution: mid, success: r.successProb });
    if (r.successProb >= targetSuccess) {
      best = mid;
      bestSuccess = r.successProb;
      hi = mid;
    } else {
      lo = mid;
    }
  ...
  return {
    contributionRequired: Math.round(best),
    successAtRequired: bestSuccess,
    delta: Math.round(best) - inputs.monthlyContribution,
    iterations: searchIterations,
    searched: trace.sort((a, b) => a.contribution - b.contribution),
    durationMs: Math.round(performance.now() - t0),
  };
}
...
export function simulateWithdrawal(
  ...
  for (let it = 0; it < iterations; it++) {
    ...
    for (let m = 1; m <= months; m++) {
      ...
      bal = bal + pnl - wd;
      if (Math.random() < monthlyBlackSwan)
        bal *= 1 - inputs.blackSwanMagnitude;
      if (bal <= 0 && ruined < 0) {
        ruined = m;
        bal = 0;
        break;
  ...
  return {
    ...
    medianYearOfRuin: ruinMonths.length
      ? percentile(
...
export function findSafeWithdrawalRate(
  ...
  for (let step = 0; step < 10; step++) {
    ...
    if (r.successProbability >= targetSurvival) {
      best = r;
      lo = mid;
    } else {
      hi = mid;
  ...
  return (
    best ?? simulateWithdrawal(initialBalance, 0.04, inputs, years, iterations)
  );
}
...
function runSimulationBiased(
  ...
  for (let it = 0; it < iterations; it++) {
    let nw = p.startingNetWorth,
      inc = p.monthlyIncome,
      exp = p.monthlyExpenses,
      contrib = p.monthlyContribution;
    for (let m = 1; m <= p.horizonMonths; m++) {
      inc *= 1 + monthlyIncomeGrowth;
      exp *= 1 + monthlyInflation;
      contrib *= 1 + monthlyIncomeGrowth;
...
export function analyzeSequenceRisk(
  ...
  const badEarly = percentile(
    runSimulationBiased(inputs, iterations, true).finals.sort((a, b) => a - b),
    0.5,
  );
  const badLate = percentile(
    runSimulationBiased(inputs, iterations, false).finals.sort((a, b) => a - b),
    0.5,
  );
  const delta = badLate - badEarly;
  const meanEnd = (badLate + badEarly) / 2;
  return {
    badEarlyEndpointP50: badEarly,
    badLateEndpointP50: badLate,
    delta,
    asymmetry: meanEnd > 0 ? Math.abs(delta) / meanEnd : 0,
  };
}

export function sampleTrajectories(
  inputs: ScenarioInputs,
  n = 8,
): { month: number; value: number; path: number }[] {
  ...
  for (let k = 0; k < n; k++) {
    ...
    for (let m = 0; m < path.length; m += every)
      out.push({ month: m, value: Math.round(path[m]), path: k });
  }
  return out;
}
