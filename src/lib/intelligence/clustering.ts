...
const ARCHETYPES: {
  name: SpendingPersonality["archetype"];
  ...
  {
    name: "Saver",
    weights: { savings: 0.6, essentials: 0.3, lifestyle: 0.1 },
    description:
      "Prioritizes long-term security and minimal discretionary spend.",
    traits: ["Frugal", "Future-focused", "Low burn-rate"],
  },
  {
    name: "Investor",
    weights: { investments: 0.5, savings: 0.3, essentials: 0.2 },
    description: "Channels surplus into wealth-building instruments.",
    traits: ["Compounder", "Asset accumulator", "Strategic"],
  },
  {
    name: "Experiencer",
    weights: { entertainment: 0.4, food: 0.3, travel: 0.3 },
    description: "Spends on memories — dining, travel and experiences.",
    traits: ["Lifestyle-rich", "Memory-maker", "Social"],
  },
  {
    name: "Provider",
    weights: { essentials: 0.5, family: 0.3, savings: 0.2 },
    description: "Focused on essentials and dependents.",
    traits: ["Stable", "Caregiver", "Pragmatic"],
  },
  {
    name: "Builder",
    weights: { home: 0.4, essentials: 0.3, savings: 0.3 },
    description: "Investing in lasting assets — home, vehicle, durables.",
    traits: ["Long-horizon", "Equity-builder"],
  },
  {
    name: "Explorer",
    weights: { travel: 0.5, entertainment: 0.3, food: 0.2 },
    description: "Heavy travel + experience footprint.",
    traits: ["Adventurous", "Globally-curious"],
  },
];
...
export function classifyPersonality(txs: Tx[]): SpendingPersonality {
  ...
  const explFeatures = [
    {
      name: "Top match",
      value: top.name,
      contribution: top.score,
      description: `Cosine similarity ${top.score.toFixed(3)}.`,
    },
    {
      name: "Runner-up",
      value: second?.name || "—",
      contribution: second?.score,
      description: `Margin ${margin.toFixed(3)} (≥0.05 = decisive).`,
    },
    {
      name: "Spend entropy",
      value: distEntropy.toFixed(2),
      description:
        "Shannon entropy across buckets — higher = more diversified.",
    },
  ...
  if (top.score < 0.5 || margin < 0.05) {
    return {
      ...
      traits: ["Diversified spend", "No dominant pattern"],
      vector,
      description:
        "Your spending is balanced across multiple lifestyle dimensions.",
      explanation: {
        algorithm: "Spending Personality",
        summary: `No dominant archetype — top match "${top.name}" only ${top.score.toFixed(2)} similarity with ${margin.toFixed(2)} margin.`,
        method:
          "Cosine similarity vs reference archetype vectors with entropy diversification check",
        ...
        diagnostics: [
          ...
          {
            label: "Distinct buckets",
            value: `${Object.keys(buckets).length}`,
          },
        ],
        confidence: top.score,
      },
