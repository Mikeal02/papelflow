...
export function detectAnomalies(txs: Tx[]): AnomalyOutput {
  ...
  const emptyExpl: AlgorithmExplanation = {
    ...
    evidence: [],
    diagnostics: [
      { label: "Sample size", value: `${expenses.length} expenses` },
    ],
    confidence: 0,
  };
  ...
  for (const [, group] of byCategory) {
    ...
    const { cleaned, outlierIndices } = hampelFilter(
      amounts,
      Math.min(7, Math.floor(amounts.length / 2)),
      3,
    );
    if (outlierIndices.length) {
      cleanedCategories++;
      contaminatedPoints += outlierIndices.length;
    }
      ...
      const features = [
        {
          ...
          value: z.toFixed(2),
          impact: Math.min(1, z / 6),
        },
        {
          name: "Amount vs median",
          value: `${(amt / Math.max(med, 1)).toFixed(1)}×`,
          impact: Math.min(1, amt / Math.max(med, 1) / 5),
        },
        {
          name: "Payee novelty",
          value: isNovel ? "New merchant" : "Recurring",
          impact: isNovel ? 0.7 : 0.2,
        },
        {
          name: "Recency",
          value: `${Math.round(ageDays)}d ago`,
          impact: recencyBoost,
        },
      ];
  ...
  const explanation: AlgorithmExplanation = {
    ...
    method:
      "Per-category Modified Z-score with Hampel pre-filtering and global IQR fallback",
    ...
    features: [
      {
        ...
        description:
          "Spending grouped by category for context-aware baselines.",
      },
      {
        name: "Hampel-cleaned categories",
        value: cleanedCategories,
        description:
          "Pre-filter neutralized contamination before computing MAD.",
      },
      {
        name: "Contaminated points removed",
        value: contaminatedPoints,
        description:
          "Extreme values winsorized so a single shock doesn't mask others.",
      },
      {
        name: "Global IQR upper fence",
        value: globalUpper.toFixed(2),
        description:
          "Q3 + 1.5·IQR — secondary signal for low-confidence categories.",
      },
    ],
    ...
    diagnostics: [
      { label: "Total expenses scanned", value: `${expenses.length}` },
      {
        ...
        value: `${top.filter((a) => a.severity === "extreme").length}E · ${top.filter((a) => a.severity === "high").length}H · ${top.filter((a) => a.severity === "medium").length}M · ${top.filter((a) => a.severity === "low").length}L`,
      },
    ],
