...
export type ActionStatus =
  "open" | "snoozed" | "pinned" | "dismissed" | "resolved";
...
const SEV_BASE: Record<ActionSeverity, number> = {
  critical: 40,
  high: 30,
  medium: 20,
  low: 10,
...
function computeScore(opts: {
  ...
  const impactRatio =
    opts.income > 0
      ? Math.abs(opts.impact) / opts.income
      : Math.min(1, Math.abs(opts.impact) / 1000);
  const impact = Math.min(25, Math.round(impactRatio * 25));
  const urg =
    opts.urgencyDays === undefined
      ? 8
      : opts.urgencyDays <= 0
        ? 20
        : opts.urgencyDays <= 3
  ...
  const recency =
    opts.ageDays <= 1 ? 5 : opts.ageDays <= 3 ? 3 : opts.ageDays <= 7 ? 2 : 1;
...
export function generateActions(opts: {
  ...
  const last30Income = transactions
    .filter(
      (t) =>
        t.type === "income" && differenceInDays(today, parseISO(t.date)) <= 30,
    )
    .reduce((s, t) => s + Number(t.amount), 0);
  const last30Expense = transactions
    .filter(
      (t) =>
        t.type === "expense" && differenceInDays(today, parseISO(t.date)) <= 30,
    )
    .reduce((s, t) => s + Number(t.amount), 0);
  ...
  for (const a of report.anomalies.slice(0, 12)) {
    ...
    const { breakdown, total } = computeScore({
      severity: sev,
      impact: a.amount,
      income: incomeBaseline,
      urgencyDays: 7 - ageDays,
      confidence: conf,
      ageDays,
    });
    actions.push({
      ...
      evidence: [
        { label: "Z-score", value: a.zScore.toString() },
        {
          ...
          value: `${a.expectedRange[0].toFixed(0)} – ${a.expectedRange[1].toFixed(0)}`,
        },
      ...
      cta: {
        ...
        intent: "open_transaction",
        route: "/transactions",
        payload: { id: a.id },
      },
      secondaryActions: [
        { label: "Open analytics", intent: "navigate", route: "/analytics" },
      ],
      detectedAt: a.date,
    });
  }
  ...
  for (const r of report.recurring) {
    ...
    const sev: ActionSeverity =
      r.totalAnnualCost > 600
        ? "high"
        : r.totalAnnualCost > 200
          ? "medium"
          : "low";
    const { breakdown, total } = computeScore({
      severity: sev,
      impact: r.totalAnnualCost,
      income: incomeBaseline * 12,
      urgencyDays: 30,
    ...
    actions.push({
      ...
      cta: {
        ...
        payload: {
          ...
          next_due: r.nextPredictedDate,
          frequency: cadenceToFreq(r.cadence),
        },
      },
      secondaryActions: [
        {
  ...
  for (const t of monthExpenses) {
    ...
    spendByCat.set(
      t.category_id,
      (spendByCat.get(t.category_id) || 0) + Number(t.amount),
    );
  }
  for (const b of budgets) {
    ...
    const over = pct >= 1;
    const sev: ActionSeverity =
      pct >= 1.25 ? "critical" : over ? "high" : "medium";
    ...
    actions.push({
      ...
      effortMinutes: 3,
      title: over
        ? `Over budget: ${b.category?.name || "Category"}`
      ...
      secondaryActions: [
        { label: "Reallocate", intent: "navigate", route: "/budgets" },
      ],
      detectedAt: todayISO,
    });
  }
  ...
  for (const s of subscriptions) {
    ...
    const sev: ActionSeverity =
      daysUntil <= 0 ? "high" : daysUntil <= 2 ? "medium" : "low";
    const { breakdown, total } = computeScore({
      severity: sev,
  ...
  const dims = Object.entries(report.health.dimensions).sort(
  ...
  if (
    report.forecast.trend === "declining" &&
    report.forecast.trendStrength > 0.25
  ) {
    const sev: ActionSeverity =
      report.forecast.trendStrength > 0.6 ? "critical" : "high";
    const { breakdown, total } = computeScore({
      severity: sev,
      impact: Math.abs(report.forecast.endOfMonthBalance),
      income: incomeBaseline,
      urgencyDays: 14,
      ...
      evidence: [
        {
          ...
          value: report.forecast.trendStrength.toFixed(2),
        },
  ...
  for (const g of goals) {
    ...
    if (last30Income > 0 && monthlyNeeded / last30Income > 0.25) {
      const sev: ActionSeverity =
        monthlyNeeded / last30Income > 0.5 ? "high" : "medium";
      const { breakdown, total } = computeScore({
        severity: sev,
        impact: monthlyNeeded,
        income: incomeBaseline,
        urgencyDays: Math.min(daysLeft, 30),
        confidence: 0.9,
        ageDays: 0,
      });
      actions.push({
        ...
        evidence: [
          ...
          {
            label: "Progress",
            value: `${Math.round((Number(g.current_amount) / Number(g.target_amount)) * 100)}%`,
          },
        ],
  ...
  if (last30Income > 0) {
    ...
    if (savingsRate < 0.15) {
      const sev: ActionSeverity =
        savingsRate < 0 ? "critical" : savingsRate < 0.05 ? "high" : "medium";
      ...
      actions.push({
        ...
        secondaryActions: [
          { label: "View analytics", intent: "navigate", route: "/analytics" },
        ],
        detectedAt: todayISO,
      });
    }
  ...
  return actions
    ...
    .map((a) =>
      pinned.has(a.id) ? { ...a, priority: Math.min(100, a.priority + 50) } : a,
    )
...
export function totalImpact(actions: PriorityAction[]): number {
  return actions.reduce(
    (s, a) => s + (a.impact && a.impact > 0 ? a.impact : 0),
    0,
  );
}

export type SortKey =
  "priority" | "impact" | "recent" | "confidence" | "urgency";
...
export function sortActions(
  ...
  switch (sort) {
    ...
    case "urgency":
      return copy.sort(
        (a, b) => (a.urgencyDays ?? 999) - (b.urgencyDays ?? 999),
      );
    default:
...
export function groupActions(
  actions: PriorityAction[],
  group: GroupKey,
): { key: string; items: PriorityAction[] }[] {
