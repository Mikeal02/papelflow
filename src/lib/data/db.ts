...
interface FinflowSchema extends DBSchema {
  ...
  keyring: {
    key: string; // 'active' or historical uuid
    value: {
      id: string;
      key: CryptoKey; // non-extractable, structured-clone-safe
      createdAt: number;
      algorithm: "AES-GCM";
      version: number;
    };
  ...
  audit_log: {
    ...
    value: {
      ...
      op:
        ...
        | "rotate-key";
      entityId: string;
      hash: string; // sha256 of payload (never stores plaintext)
      meta?: Record<string, any>;
    };
    indexes: { "by-at": number; "by-table": string };
  };
}
  ...
  dbPromise = openDB<FinflowSchema>(dbName(userId), DB_VERSION, {
    upgrade(db, oldVersion, _newVersion, tx) {
      ...
      if (oldVersion < 3) {
        ...
        if (!q.indexNames.contains("by-entity"))
          q.createIndex("by-entity", "entityKey");
