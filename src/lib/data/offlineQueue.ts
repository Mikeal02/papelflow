...
export function laneKey(table: string, entityId: string) {
...
async function nextSeq(userId: string): Promise<number> {
  if (!seqSeeded) {
    try {
      ...
      seqCounter = all.reduce(
        (m, r) => Math.max(m, r.seq ?? r.createdAt ?? 0),
        0,
      );
    } catch {
      /* empty queue */
...
export type EnqueueInput = Omit<
  QueuedMutation,
  "attempts" | "nextAttemptAt" | "createdAt" | "id" | "entityKey" | "seq"
> & { id?: string };
...
export async function enqueue(m: EnqueueInput): Promise<QueuedMutation | null> {
  ...
  const lane = (
    await db.getAllFromIndex("mutation_queue", "by-entity", key)
  ...
  if (
    m.op === "update" &&
    foldable &&
    (foldable.op === "insert" || foldable.op === "update")
  ) {
...
export async function drain(userId: string) {
  ...
  try {
    ...
    while (guard++ < 200) {
      const now = Date.now();
      const all = (await db.getAll("mutation_queue")).sort(
        (a, b) => (a.seq ?? 0) - (b.seq ?? 0),
      );
      ...
      const results = await Promise.all(
        frontier.map(async (m) => ({ m, res: await apply(m) })),
      );
...
export function bindOnlineEvents(userId: string) {
  ...
  const onVisible = () => {
    if (document.visibilityState === "visible") void drain(userId);
  };
...
export async function listQueue(userId: string): Promise<QueuedMutation[]> {
  const db = await getDB(userId);
  return (await db.getAll("mutation_queue")).sort(
    (a, b) => (a.seq ?? 0) - (b.seq ?? 0),
  );
}
