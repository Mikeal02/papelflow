...
function addFooter(doc: jsPDF) {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text("Finflow Financial Platform • Confidential", 105, 294, {
      align: "center",
    });
...
export function generateFinancialReportPDF(
  ...
  y = addKPIRow(
    ...
    [
      {
        label: "Total Income",
        value: fmt(stats.totalIncome, currencySymbol),
        color: COLORS.income,
      },
      {
        label: "Total Expenses",
        value: fmt(stats.totalExpenses, currencySymbol),
        color: COLORS.expense,
      },
      {
        label: "Net Savings",
        value: fmt(stats.totalIncome - stats.totalExpenses, currencySymbol),
        color:
...
export function generateTaxDocumentPDF(
  ...
  y = addKPIRow(
    ...
    [
      { label: "Gross Income", value: fmt(annualIncome, currencySymbol) },
      {
        ...
        value: fmt(projectedTax, currencySymbol),
        color: COLORS.expense,
      },
      {
      ...
      },
      {
        ...
        value: fmt(quarterlyPayment, currencySymbol),
      },
    ],
    y,
  );
  ...
  const computationRows = [
    ...
    [
      balanceDue >= 0 ? "Balance Due" : "Refund Expected",
      fmt(Math.abs(balanceDue), currencySymbol),
    ],
  ];
  ...
  if (brackets.length > 0) {
    ...
    autoTable(doc, {
      ...
      body: brackets.map((b) => [
        ...
        fmt(b.taxed, currencySymbol),
        fmt(b.tax, currencySymbol),
      ]),
      foot: [
        [
          "Total",
          "",
          fmt(taxableIncome, currencySymbol),
          fmt(projectedTax, currencySymbol),
        ],
      ],
      theme: "striped",
      headStyles: { fillColor: COLORS.primary, fontSize: 8, fontStyle: "bold" },
  ...
  if (deductions.filter((d) => d.amount > 0).length > 0) {
    ...
    autoTable(doc, {
      ...
      body: deductions
        .filter((d) => d.amount > 0)
        .map((d) => [d.name, d.category, fmt(d.amount, currencySymbol)]),
  ...
  doc.setTextColor(...COLORS.muted);
  doc.text(
    "DISCLAIMER: This document is for estimation purposes only and does not constitute tax advice.",
    15,
    y,
  );
  doc.text(
    "Please consult a qualified tax professional for official tax preparation and filing.",
    15,
    y + 4,
  );
  ...
  addFooter(doc);
  doc.save(
    `Finflow_Tax_Estimate_${year}_${format(new Date(), "yyyy-MM-dd")}.pdf`,
  );
}
...
export function generatePortfolioStatementPDF(
  ...
  const kpis = [
    ...
    { label: "Total Cost", value: fmt(totalCost, currencySymbol) },
    {
      ...
      value: fmt(totalGain, currencySymbol),
      color: totalGain >= 0 ? COLORS.income : COLORS.expense,
    },
    {
      label: "Total Return",
      value: `${totalReturn.toFixed(1)}%`,
      color: totalReturn >= 0 ? COLORS.income : COLORS.expense,
    },
    {
      label: "Annual Dividends",
      value: fmt(annualDividends, currencySymbol),
      color: COLORS.accent,
    },
  ...
  autoTable(doc, {
    ...
    foot: [
      [
        ...
        "",
        "",
        fmt(totalValue, currencySymbol),
        fmt(totalGain, currencySymbol),
  ...
  if (allocation.length > 0 && y < 160) {
    ...
    autoTable(doc, {
      ...
      body: allocation.map((a) => [
        a.name,
        fmt(a.value, currencySymbol),
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      "Finflow Financial Platform • Confidential • For Personal Use Only",
      148,
      209,
      { align: "center" },
    );
  ...
  doc.save(
    `Finflow_Portfolio_Statement_${format(new Date(), "yyyy-MM-dd")}.pdf`,
  );
}
