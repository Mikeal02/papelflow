...
export interface SpendingAnomaly {
  id: string;
  type:
    "spike" | "unusual_time" | "new_merchant" | "category_surge" | "velocity";
...
export function useAdvancedAnalytics() {
  ...
  const velocityMetrics = useMemo((): VelocityMetrics => {
    ...
    const daysUntilBudgetExhausted =
      currentVelocity > 0 && totalBudget > monthExpenses
  ...
  const anomalies = useMemo((): SpendingAnomaly[] => {
    ...
    const avgTransaction =
      last30Days.length > 0
        ? last30Days.reduce((s, t) => s + Number(t.amount), 0) /
          last30Days.length
        : 0;
    ...
    const avgWeekday =
      weekdaySpend.length > 0
        ? weekdaySpend.reduce((s, t) => s + Number(t.amount), 0) /
          weekdaySpend.length
        : 0;
    const avgWeekend =
      weekendSpend.length > 0
        ? weekendSpend.reduce((s, t) => s + Number(t.amount), 0) /
          weekendSpend.length
        : 0;
    ...
    categories
      ...
      .forEach((cat) => {
        const currentSpend = transactions
          .filter(
            (t) => t.category_id === cat.id && t.date?.startsWith(currentMonth),
          )
          .reduce((s, t) => s + Number(t.amount), 0);
        const lastSpend = transactions
          .filter(
            (t) => t.category_id === cat.id && t.date?.startsWith(lastMonth),
          )
          .reduce((s, t) => s + Number(t.amount), 0);

        if (
          lastSpend > 0 &&
          currentSpend > lastSpend * 1.5 &&
          currentSpend > 100
        ) {
          result.push({
  ...
  const heatmapData = useMemo((): HeatmapData[] => {
    ...
    return days.map((day) => {
      ...
      const dayTx = transactions.filter(
        (t) => t.date === dayStr && t.type === "expense",
      );
  ...
  const timeOfDayData = useMemo((): TimeOfDayData[] => {
    const hourBuckets: { amount: number; count: number }[] = Array(24)
      .fill(null)
  ...
  const dayOfWeekData = useMemo((): DayOfWeekData[] => {
    const dayBuckets: { amount: number; count: number }[] = Array(7)
      .fill(null)
      .map(() => ({ amount: 0, count: 0 }));
    const dayNames = [
      "Sunday",
  ...
  const merchantInsights = useMemo((): MerchantInsight[] => {
    ...
    return Object.entries(merchantMap)
      .map(([name, data]) => {
        ...
        const olderAvg =
          data.dates.slice(0, midPoint).length > 0
            ? data.total / data.dates.slice(0, midPoint).length
            : 0;
        const recentAvg =
          data.dates.slice(midPoint).length > 0
  ...
  const financialPulse = useMemo(() => {
    ...
    const anomalyPenalty =
      anomalies.filter((a) => a.severity === "high").length * 10;
