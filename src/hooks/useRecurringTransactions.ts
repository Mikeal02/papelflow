...
export const useRecurringTransactions = () => {
  ...
  const isAlreadyProcessed = useCallback(
    (subscriptionId: string, date: string): boolean => {
      const processed = getProcessedTransactions();
  ...
  const getNextDueDate = useCallback(
    (currentDue: Date, frequency: string): Date => {
      switch (frequency) {
        case "weekly":
  ...
  const processRecurringTransactions = useCallback(async () => {
    ...
    for (const subscription of activeSubscriptions) {
      ...
      while (isBefore(dueDate, addDays(today, 1)) && iterationCount < 12) {
        ...
        try {
          ...
          console.error(
            `Failed to create recurring transaction for ${subscription.name}:`,
            error,
          );
        }
      ...
      if (newNextDue !== subscription.next_due) {
        try {
          ...
          console.error(
            `Failed to update next_due for ${subscription.name}:`,
            error,
          );
        }
      }
    ...
    if (processedCount > 0) {
      toast.success(
        `${processedCount} recurring transaction${processedCount > 1 ? "s" : ""} auto-generated`,
      );
    }
