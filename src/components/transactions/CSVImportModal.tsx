  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (lines.length < 2) {
        toast({
          title: "Invalid CSV",
          description: "File must have at least a header and one data row.",
          variant: "destructive",
        });
        return;
      }
  ...
  const handleImport = async () => {
    if (!selectedAccount) {
      toast({
        ...
        description: "Please select an account to import transactions into.",
        variant: "destructive",
      });
      return;
    }
            ...
            >
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Supports .csv files with date, amount, type columns
                </p>
              </div>
              <input
                ref={fileRef}
                    ...
                    {parsedRows.slice(0, 10).map((row, i) => (
                      <tr key={i} className="border-t border-border/50">
                        ...
                        <td className="p-2">
                          <Badge
                            variant={
                              row.type === "income" ? "default" : "secondary"
                            }
                            className="text-[10px]"
                          >
                            {row.type}
