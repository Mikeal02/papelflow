...
export function BudgetReallocation({
  ...
  const currentTotal = Object.values(localAmounts).reduce(
    (sum, a) => sum + a,
    0,
  );
  const hasChanges = budgets.some(
    (b) => localAmounts[b.id] !== Number(b.amount),
  );
  ...
  const saveChanges = async () => {
    ...
    try {
      const updates = budgets
        ...
        .map((b) =>
          updateBudget.mutateAsync({ id: b.id, amount: localAmounts[b.id] }),
        );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Drag & drop to move money between categories
            </p>
          </div>
        </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative h-2 rounded-full bg-muted/50 overflow-hidden">
                  <motion.div
                    ...
                    initial={{ width: 0 }}
                    animate={{
                      width: `${Math.min((budget.spent / currentAmount) * 100, 100)}%`,
                    }}
                  />
                </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-center gap-3 mb-6">
                <div className="text-center">
                  <div
                    ...
                    style={{
                      backgroundColor: `${sourceBudget.category?.color}20`,
                    }}
                  >
                    {getCategoryIcon(sourceBudget.category?.icon)}
                  </div>
                  <p className="text-sm font-medium truncate max-w-[100px]">
                    {sourceBudget.category?.name}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {formatCurrency(localAmounts[sourceBudget.id])}
                  </p>
                </div>
                ...
                <div className="text-center">
                  <div
                    className="h-12 w-12 mx-auto rounded-xl flex items-center justify-center text-xl mb-2"
                    style={{
                      backgroundColor: `${targetBudget.category?.color}20`,
                    }}
                  >
                    {getCategoryIcon(targetBudget.category?.icon)}
                  </div>
                  <p className="text-sm font-medium truncate max-w-[100px]">
                    {targetBudget.category?.name}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {formatCurrency(localAmounts[targetBudget.id])}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  {[25, 50, 100, localAmounts[sourceBudget.id]].map(
                    (amount, i) => (
                      <Button
                        key={i}
                        variant="outline"
                        ...
                        onClick={() =>
                          setTransferAmount(
                            String(
                              Math.min(amount, localAmounts[sourceBudget.id]),
                            ),
                          )
                        }
                      >
