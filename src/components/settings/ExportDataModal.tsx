...
export function ExportDataModal({
  ...
  const handleExport = async () => {
    ...
    if (selectedFormat === "json") {
      ...
      if (selectedModules.has("accounts")) {
        ...
        data.accounts.forEach((a) => {
          csv +=
            [a.name, a.type, a.balance, a.currency || "USD", a.notes || ""]
              .map((v) => `"${String(v).replace(/"/g, '""')}"`)
              .join(",") + "\n";
        });
        csv += "\n";
      ...
      if (selectedModules.has("budgets")) {
        ...
        data.budgets.forEach((b) => {
          csv +=
            [b.month, b.category_id, b.amount, b.rollover ? "Yes" : "No"]
              .map((v) => `"${String(v).replace(/"/g, '""')}"`)
              .join(",") + "\n";
        });
        csv += "\n";
      ...
      if (selectedModules.has("goals")) {
        ...
        data.goals.forEach((g) => {
          csv +=
            [g.name, g.target_amount, g.current_amount || 0, g.deadline || ""]
              .map((v) => `"${String(v).replace(/"/g, '""')}"`)
              .join(",") + "\n";
        });
        csv += "\n";
    ...
    toast({
      ...
      description: `${totalRecords} records exported as ${selectedFormat.toUpperCase()}`,
    });
    setTimeout(() => {
      setExported(false);
      onOpenChange(false);
          ...
          {exported ? (
            ...
            >
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {totalRecords} records • {estimateSize} •{" "}
            ...
            >
              <DialogHeader className="p-6 pb-0">
                ...
                <DialogDescription>
                  Choose a format and select which data to include in your
                  export.
                </DialogDescription>
              </DialogHeader>
