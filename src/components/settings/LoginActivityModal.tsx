...
export function LoginActivityModal({ open, onOpenChange }: Props) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) =>
          setEvents((prev) =>
            [payload.new as LoginEvent, ...prev].slice(0, 50),
          ),
      )
      .subscribe();
    return () => {
  ...
  const stats = useMemo(() => {
    ...
    const uniqueCountries = new Set(
      signIns.map((e) => e.country).filter(Boolean),
    ).size;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl p-0 overflow-hidden">
        <DialogHeader className="px-6 pt-6 pb-4">
          ...
          <DialogDescription>
            Review recent sign-ins, sessions, and security events on your
            account.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="px-6 grid grid-cols-4 gap-2">
          {[
            ...
            { label: "Countries", value: stats.uniqueCountries },
            {
              ...
              tone: stats.suspicious ? "text-destructive" : "",
            },
          ].map((s) => (
            <div
              key={s.label}
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Every other device signed in to your account will be signed
                  out immediately. Your current session stays active.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This permanently deletes all recorded login events for your
                  account. New events will still be captured.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
        ...
        <ScrollArea className="h-[420px] px-6 py-4">
          {loading ? (
            ...
            <div className="text-center py-16 text-sm text-muted-foreground">
              No login activity recorded yet. Events will appear here on your
              next sign-in.
            </div>
          ) : (
            <ol className="relative border-l border-border/60 ml-2 space-y-3">
              <AnimatePresence initial={false}>
                {events.map((e, i) => {
                  ...
                  const isCurrent =
                    !!currentSessionTag &&
                    e.session_id === currentSessionTag &&
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {isCurrent && (
                                <Badge className="bg-income/15 text-income border-income/30 gap-1 text-[10px]">
                                  <CheckCircle2 className="h-2.5 w-2.5" /> This
                                  device
                                </Badge>
                              )}
                              {e.is_suspicious && !isCurrent && (
                                <Badge
                                  variant="outline"
                                  className="text-[10px] text-destructive border-destructive/40 gap-1"
                                >
                                  <ShieldAlert className="h-2.5 w-2.5" /> New
                                  location
                                </Badge>
                              )}
                            </div>
                            ...
                            <div className="mt-1.5 flex items-center flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground">
                              ...
                              {e.ip_address && (
                                <span className="flex items-center gap-1">
                                  <Globe2 className="h-3 w-3" /> {e.ip_address}
                                </span>
                              )}
                            </div>
                          </div>
