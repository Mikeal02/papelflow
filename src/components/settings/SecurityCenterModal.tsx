...
const KIND_META: Record<
  string,
  { label: string; icon: typeof ShieldCheck; hint: string }
> = {
  rate_limit_exceeded: {
...
export function SecurityCenterModal({ open, onOpenChange }: Props) {
  ...
  const posture = useMemo(() => {
    ...
    if (warn > 0)
      return { label: "Monitoring", tone: "text-amber-400", icon: ShieldAlert };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl p-0 overflow-hidden">
        <DialogHeader className="px-6 pt-6 pb-4">
          ...
          <DialogDescription>
            Append-only, server-written record of sensitive activity on your
            account. These entries cannot be edited or deleted from the app.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="px-6 pb-4 flex flex-wrap items-center gap-2">
          <div
            className={cn(
              "flex items-center gap-2 rounded-xl border border-border/60 bg-card/60 px-3 py-2 text-sm font-medium",
              posture.tone,
            )}
          >
            ...
            {!loading && filtered.length === 0 && (
              <div className="flex flex-col items-center gap-2 py-14 text-center">
                ...
                <p className="max-w-sm text-xs text-muted-foreground">
                  No throttling, rejected credentials or unusual sign-ins have
