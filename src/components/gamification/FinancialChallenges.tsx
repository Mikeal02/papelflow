...
export const FinancialChallenges = () => {
  ...
  useEffect(() => {
    if (completedCount > 0) {
      const lastCount = parseInt(
        localStorage.getItem("lastCompletedCount") || "0",
      );
  ...
  const challenges: Challenge[] = useMemo(() => {
    ...
    const savingsRate = stats?.income
      ...
      : 0;
    const monthlyExpensesByCategory = monthExpenses.reduce(
      (acc, t) => {
        if (t.category_id)
    ...
    return [
      ...
      },
      {
        ...
        reward: "+150 XP",
        status:
          budgetsOnTrack === budgets.length && budgets.length > 0
            ? "completed"
            : budgets.length > 0
              ? "active"
              : "locked",
      ...
      },
      {
        ...
        reward: "+250 XP",
        status:
          new Set(accounts.map((a) => a.type)).size >= 3
            ? "completed"
            : "active",
        xp: 250,
      },
  ...
  const badges: AchievementBadge[] = useMemo(
    () => [
      ...
      },
      {
        ...
        name: "Finance Master",
        icon: Trophy,
        description: "Log 200 transactions",
        earned: transactions.length >= 200,
        tier: "gold",
      },
      {
        id: "streak-30",
      ...
      },
      {
        ...
        name: "Diversified",
        icon: Sparkles,
        description: "3+ account types",
        earned: new Set(accounts.map((a) => a.type)).size >= 3,
        tier: "silver",
      },
    ],
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                {
                  label: "This Week",
                  value: challenges.filter((c) => c.status === "completed")
                    .length,
                  icon: Star,
                  color: "text-income",
                },
                ...
                >
                  <item.icon
                    className={cn("h-6 w-6 mx-auto mb-1", item.color)}
                  />
