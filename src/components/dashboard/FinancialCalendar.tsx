...
export function FinancialCalendar() {
  ...
  const events = useMemo(() => {
    ...
    const incomeTransactions = transactions.filter(
      (t) => t.type === "income" && t.is_recurring,
    );
    const uniquePayees = new Set(
      incomeTransactions.map((t) => t.payee).filter(Boolean),
    );
            ...
            {[
              {
                icon: CreditCard,
                label: "Bills",
                value: formatCurrency(summary.bills),
                color: "text-expense",
              ...
              <div
                key={item.label}
                className={cn(
                  "p-2.5 rounded-xl bg-gradient-to-br text-center border border-border/20",
                  item.bg,
                )}
              >
                <item.icon
                  className={cn("h-3.5 w-3.5 mx-auto mb-1", item.color)}
                />
          ...
          <div className="space-y-[3px]">
            <div className="grid grid-cols-7 gap-[3px]">
              {weekDays.map((day) => (
                <div
                  key={day}
                  className="text-center text-[9px] font-semibold text-muted-foreground uppercase tracking-wider py-1"
                >
                  {day}
                </div>
              ))}
                  ...
                  >
                    ...
                    {hasEvents && (
                      <div className="flex gap-[2px] justify-center mt-0.5">
                        {hasBill && (
                          <div className="h-[4px] w-[4px] rounded-full bg-expense" />
                        )}
                        {hasIncome && (
                          <div className="h-[4px] w-[4px] rounded-full bg-income" />
                        )}
                        {hasGoal && (
            ...
            {selectedDay && selectedDayEvents.length > 0 ? (
              ...
              >
                ...
                {selectedDayEvents.map((event) => (
                  <div
                    key={event.id}
                    className="flex items-center justify-between gap-2 text-xs p-2 rounded-lg bg-muted/20 border border-border/20"
                  >
