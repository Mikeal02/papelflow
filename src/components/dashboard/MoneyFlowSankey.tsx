...
export const MoneyFlowSankey = memo(function MoneyFlowSankey() {
  ...
  const { incomeNodes, expenseNodes, links, totalIncome, totalExpenses } =
    useMemo(() => {
      const now = new Date();
      ...
      monthTx.forEach((t) => {
        ...
        const catName =
          cat?.name || (t.type === "income" ? "Other Income" : "Uncategorized");
        const catColor =
          cat?.color || (t.type === "income" ? "#10B981" : "#6366F1");
        const key = t.category_id || `uncategorized-${t.type}`;

        if (t.type === "income") {
          if (!incomeByCat[key])
            incomeByCat[key] = { name: catName, amount: 0, color: catColor };
          incomeByCat[key].amount += Number(t.amount);
        } else {
          if (!expenseByCat[key])
            expenseByCat[key] = { name: catName, amount: 0, color: catColor };
  ...
  const svgHeight = Math.max(
    300,
    Math.max(incomeNodes.length, expenseNodes.length) * 50 + 40,
  );
  const nodeWidth = 14;
  const leftX = 40;
  ...
  const getNodePositions = (nodes: FlowNode[], x: number) => {
    ...
    return nodes.map((node) => {
      const height = Math.max(
        16,
        (node.value / totalValue) * usableHeight - padding,
      );
  ...
  const generatePath = (
    sourcePos: (typeof leftPositions)[0],
    targetPos: (typeof rightPositions)[0],
    sourceOffset: number,
    targetOffset: number,
    thickness: number,
  ) => {
  ...
  const linkPaths = links
    .map((link) => {
      ...
      const targetPct =
        totalExpenses > 0 ? link.value / targetPos.node.value : 0;
          ...
          >
            <defs>
              {leftPositions.map((pos) => (
                <linearGradient
                  key={`grad-${pos.node.id}`}
                  id={`flow-grad-${pos.node.id}`}
                  x1="0"
                  x2="1"
                  y1="0"
                  y2="0"
              ...
              (item, i) =>
                item && (
                  <path
                    key={i}
                    d={item.path}
                    fill={`url(#flow-grad-${item.link.source})`}
