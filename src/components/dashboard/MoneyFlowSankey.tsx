...
export function MoneyFlowSankey() {
  ...
  const { incomeNodes, expenseNodes, links, totalIncome, totalExpenses } = useMemo(() => {
    ...
    // Create links: each income → each expense proportionally
    ...
    incomeNodes.forEach(iNode => {
      ...
      expenseNodes.forEach(eNode => {
        ...
        if (flowValue > 0) {
          links.push({ source: iNode.id, target: eNode.id, value: flowValue });
        }
      });
    });
  ...
  const generatePath = (sourcePos: typeof leftPositions[0], targetPos: typeof rightPositions[0], sourceOffset: number, targetOffset: number, thickness: number) => {
    ...
    return `M ${x1},${y1} C ${cx1},${y1} ${cx2},${y2} ${x2},${y2} 
            L ${x2},${y2 + thickness} C ${cx2},${y2 + thickness} ${cx1},${y1 + thickness} ${x1},${y1 + thickness} Z`;
  };
  ...
  const linkPaths = links.map((link, i) => {
    ...
    const path = generatePath(sourcePos, targetPos, sOffset, tOffset, Math.max(sourceThickness, targetThickness));

    return { path, color: sourcePos.node.color, link, index: i };
  }).filter(Boolean);
      ...
      <CardHeader className="pb-2">
        ...
        {savings > 0 && (
          <motion.div
            ...
            className="text-xs text-income font-medium"
          >
          ...
          >
            ...
            {linkPaths.map((item, i) => item && (
              <motion.path
                ...
                transition={{ delay: 0.3 + i * 0.03, duration: 0.8, ease: 'easeOut' }}
