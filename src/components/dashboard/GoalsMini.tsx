...
export function GoalsMini() {
  ...
  const completedGoals = goals.filter(
    (g) => Number(g.current_amount || 0) >= Number(g.target_amount),
  ).length;
  ...
  return (
    ...
    >
      <TiltCard
        intensity={8}
        className="stat-card h-full relative overflow-hidden"
      >
        ...
        <div className="relative flex items-center justify-between gap-2 mb-5">
          ...
          <Link to="/goals" className="shrink-0">
            <Button
              variant="ghost"
              size="sm"
              className="text-primary hover:text-primary/80 h-8 gap-1"
            >
              View all
          ...
          {activeGoals.length === 0 ? (
            ...
            >
              {activeGoals.map((goal, index) => {
                ...
                const isUrgent =
                  daysRemaining !== null && daysRemaining <= 30 && !isComplete;
