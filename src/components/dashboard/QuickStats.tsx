...
export const QuickStats = memo(function QuickStats() {
  ...
  const quickStats = useMemo(() => {
    ...
    const onTrackBudgets = budgets.filter(
      (b) => (monthExpenses[b.category_id] || 0) <= Number(b.amount),
    ).length;
    const budgetHealth =
      budgets.length > 0
        ? Math.round((onTrackBudgets / budgets.length) * 100)
        : 100;
    ...
    return [
      ...
      },
      {
        ...
        progressColor:
          budgetHealth >= 80 ? ("income" as const) : ("warning" as const),
      },
      {
        label: "Transactions",
      ...
      {quickStats.map((stat, i) => (
        ...
        >
          ...
          <div className="relative flex items-center justify-between mb-2">
            <div
              className={cn(
                "flex items-center justify-center h-7 w-7 rounded-lg bg-background/60 backdrop-blur-sm border border-border/40",
              )}
            >
              <stat.icon
                className={cn("h-3.5 w-3.5 shrink-0", stat.iconColor)}
              />
            </div>
          ...
          <div className="relative min-w-0 overflow-hidden">
            <CountUpValue
              value={stat.value}
              className="text-sm sm:text-base md:text-lg font-bold block truncate tnum"
              duration={1000}
            />
            <p className="text-[10px] text-muted-foreground/80 truncate uppercase tracking-wider">
              {stat.sub || stat.label}
            </p>
          </div>
        </motion.div>
