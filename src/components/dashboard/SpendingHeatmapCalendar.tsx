...
const getHeatColor = (intensity: number, isDark: boolean) => {
  if (intensity === 0)
    return isDark ? "hsl(222 25% 10% / 0.5)" : "hsl(215 25% 96% / 0.5)";
...
export function SpendingHeatmapCalendar() {
  ...
  const { days, maxSpend, totalSpend, avgSpend, topDay, weekdayTotals } =
    useMemo(() => {
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            {
              label: "Peak",
              value: topDay ? format(topDay.date, "MMM d") : "—",
              className: "text-expense",
            },
          ].map((s) => (
            <div
              key={s.label}
              className="text-center p-2 rounded-xl bg-muted/20 border border-border/20"
            >
              <p className="text-[9px] text-muted-foreground font-medium uppercase tracking-wider">
                {s.label}
              </p>
              <p className={cn("text-xs font-bold tabular-nums", s.className)}>
                {s.value}
              </p>
            </div>
          ))}
          ...
          <div className="space-y-1">
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(
              (name, i) => {
                const pct =
                  maxWeekdayTotal > 0
                    ? (weekdayTotals[i] / maxWeekdayTotal) * 100
                    : 0;
                return (
                  <div
                    key={name}
                  ...
                  >
                    <span className="w-7 text-muted-foreground font-medium">
                      {name}
                    </span>
                    <div className="flex-1 h-1.5 bg-muted/30 rounded-full overflow-hidden">
                      <motion.div
                        ...
                        style={{
                          background: `linear-gradient(90deg, hsl(var(--expense) / 0.35), hsl(var(--expense) / 0.75))`,
                        }}
                      />
                    </div>
