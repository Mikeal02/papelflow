...
export const FinancialHealthScore = memo(function FinancialHealthScore() {
  ...
  const { overallScore, metrics, rating, ratingColor, segments } =
    useMemo(() => {
      ...
      if (savingsRate >= 30) {
        ...
        savingsTip = "Excellent savings rate!";
      } else if (savingsRate >= 20) {
        savingsScore = 25;
        savingsStatus = "good";
        savingsTip = "Great job!";
      } else if (savingsRate >= 10) {
        savingsScore = 15;
        savingsStatus = "warning";
        savingsTip = "Try to increase to 20%";
      ...
      const onBudgetCount = budgetsWithSpending.filter(
        (b) => (monthlyExpenses[b.category_id] || 0) <= Number(b.amount),
      ).length;
      ...
      const budgetScore =
        budgetsWithSpending.length === 0
          ? 10
          : Math.round((budgetAdherence / 100) * 25);
      const budgetStatus: HealthMetric["status"] =
        budgetAdherence >= 90
          ? "excellent"
          : budgetAdherence >= 75
            ? "good"
      ...
      const goalScore =
        goalsWithProgress.length === 0
          ? 10
          : Math.round((avgGoalProgress / 100) * 25);
      const goalStatus: HealthMetric["status"] =
        avgGoalProgress >= 75
          ? "excellent"
          : avgGoalProgress >= 50
            ? "good"
      ...
      metricsList.push({
        ...
        maxScore: 25,
        status: goalStatus,
        tip: goalStatus === "excellent" ? "Almost there!" : "Stay consistent",
      ...
      const diversityStatus: HealthMetric["status"] =
        diversityScore >= 15
          ? "excellent"
          : diversityScore >= 10
            ? "good"
          ...
          <div className="flex items-center gap-4">
            <div className="relative shrink-0">
              <MultiProgressRing segments={segments} size={72} strokeWidth={8}>
                <span
                  className={cn("text-xl font-bold tabular-nums", ratingColor)}
                >
                  {overallScore}
                </span>
              </MultiProgressRing>
            ...
            <div className="flex-1 min-w-0">
              <p className={cn("text-lg font-bold truncate", ratingColor)}>
                {rating}
              </p>
