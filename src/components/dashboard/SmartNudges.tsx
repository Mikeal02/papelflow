...
export const SmartNudges = memo(function SmartNudges() {
  ...
  const nudges = useMemo(() => {
    ...
    monthExpenses.forEach((t) => {
      if (t.category_id)
        categorySpend[t.category_id] =
          (categorySpend[t.category_id] || 0) + Number(t.amount);
    });
    ...
    const savingsRate =
      (stats?.income || 0) > 0 ? (surplus / (stats?.income || 1)) * 100 : 0;
    if (savingsRate >= 20) {
      result.push({
    ...
    if (activeUnseen.length > 0) {
      const totalWaste = activeUnseen.reduce(
      ...
      result.push({
        ...
        description: `Review ${activeUnseen
          .map((s) => s.name)
          .slice(0, 3)
          .join(", ")} — could save ${formatCurrency(totalWaste)}/mo`,
        icon: Zap,
        type: "tip",
        priority: 6,
      });
