...
const Table = React.forwardRef<HTMLTableElement, React.HTMLAttributes<HTMLTableElement>>(
  ({ className, ...props }, ref) => (
    <div className="relative w-full overflow-auto rounded-xl border border-border/60">
      <table
        ref={ref}
        className={cn("w-full caption-bottom border-collapse text-sm [&_td]:text-numeric-none", className)}
        {...props}
      />
    </div>
  ),
...
const TableHeader = React.forwardRef<HTMLTableSectionElement, React.HTMLAttributes<HTMLTableSectionElement>>(
  ({ className, ...props }, ref) => (
    <thead
      ref={ref}
      className={cn("sticky top-0 z-10 bg-muted/60 backdrop-blur [&_tr]:border-b [&_tr]:border-border/60", className)}
      {...props}
    />
  ),
);
