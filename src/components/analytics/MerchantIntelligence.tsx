...
import {
  ...
  EliteMerchant,
  MerchantSegment,
} from "@/lib/intelligence/merchantElite";
...
> = {
  Champion: {
    color: "text-income border-income/40 bg-income/10",
    icon: Crown,
    label: "Champion",
  },
  Loyal: {
  ...
  Potential: {
    color: "text-chart-3 border-chart-3/40 bg-chart-3/10",
    icon: Sparkles,
    label: "Potential",
  },
  New: {
    color: "text-chart-4 border-chart-4/40 bg-chart-4/10",
    icon: Target,
    label: "New",
  },
  AtRisk: {
  ...
  Lost: {
    color: "text-expense border-expense/40 bg-expense/10",
    icon: TrendingDown,
    label: "Lost",
  },
};
...
export const MerchantIntelligence = () => {
  ...
  const report = useMemo(
    () => runMerchantElite(transactions as any),
    [transactions],
  );
        ...
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">
              {report.merchants.length} merchants · {report.graph.length}{" "}
              co-visit edges
            </p>
          </div>
            ...
            .map(([seg, count]) => {
              ...
              return (
                <button
                  key={seg}
                  onClick={() =>
                    setSelectedSegment((s) => (s === seg ? "All" : seg))
                  }
                  className={cn(
      ...
      <CardContent>
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList className="grid grid-cols-4 h-8">
            ...
            <TabsTrigger value="rfm" className="text-xs">
              RFM
            </TabsTrigger>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="grid grid-cols-4 gap-2 mt-2 text-[10px]">
                        ...
                        <div>
                          ...
                          <div className="font-semibold tabular-nums">
                            {m.meanInterval > 0
                              ? `${Math.round(m.meanInterval)}d`
                              : "—"}
                          </div>
                        </div>
                        <div>
                          <div className="text-muted-foreground">CLV 12m</div>
                          <div className="font-semibold tabular-nums text-income">
                            {formatCurrency(m.clv12m)}
                          </div>
                        </div>
                      </div>
                        ...
                        <div>
                          <div className="flex items-center justify-between text-[9px] text-muted-foreground">
                            ...
                            <span className="tabular-nums">
                              {Math.round(m.churnRisk * 100)}%
                            </span>
                          </div>
                          <Progress
                            value={m.churnRisk * 100}
                            className={cn(
                              "h-1 mt-0.5",
                              m.churnRisk > 0.6 && "[&>div]:bg-expense",
                            )}
                          />
                        </div>
                        <div>
                          <div className="flex items-center justify-between text-[9px] text-muted-foreground">
                            ...
                            <span className="tabular-nums">
                              {Math.round(m.retention90d * 100)}%
                            </span>
                          </div>
                          <Progress
                            value={m.retention90d * 100}
                            className="h-1 mt-0.5 [&>div]:bg-income"
                          />
                        </div>
                      </div>
                              ...
                              m.nextVisitWindow && (
                                <>
                                  ...
                                  <span className="text-muted-foreground/70">
                                    {" "}
                                    ({Math.round(m.nextVisitWindow[0])}–
                                    {Math.round(m.nextVisitWindow[1])}d)
                                  </span>
                                </>
                              )}
                          </span>
          ...
          <TabsContent value="rfm" className="mt-3">
            ...
            <p className="text-[10px] text-muted-foreground text-center mt-1">
              Bubble size = monetary value · Color = segment · X reversed
              (recent on right)
            </p>
          </TabsContent>
            ...
            {report.topRelationships.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-xs text-muted-foreground">
                  No strong co-visit patterns yet
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        <div
                          className={cn(
                            "h-7 w-7 rounded-lg bg-gradient-to-br flex items-center justify-center text-white text-[10px] font-bold shrink-0",
                            colorFromName(r.merchant),
                          )}
                        >
                          {initials(r.merchant)}
                        </div>
                        ...
                        <div className="text-xs min-w-0">
                          <div className="font-medium truncate">
                            {r.merchant} ↔ {r.partner}
                          </div>
                          <div className="text-[10px] text-muted-foreground">
                            Jaccard {r.weight.toFixed(2)}
                          </div>
                        </div>
                      </div>
          ...
          <TabsContent value="loyalty" className="mt-3 space-y-3">
            <div className="grid grid-cols-3 gap-2">
              <div className="rounded-lg border border-border/40 bg-muted/20 p-2.5">
                ...
                <div className="text-lg font-bold tabular-nums">
                  {(report.loyaltyHHI * 10000).toFixed(0)}
                </div>
              ...
              <div className="rounded-lg border border-border/40 bg-muted/20 p-2.5">
                ...
                <div className="text-lg font-bold tabular-nums text-primary">
                  {Math.round(report.diversityIndex * 100)}%
                </div>
              ...
              <div className="rounded-lg border border-border/40 bg-muted/20 p-2.5">
                ...
                <div className="text-lg font-bold tabular-nums">
                  {formatCurrency(report.totalLifetime)}
                </div>
                <div className="text-[9px] text-muted-foreground">
                  All merchants
                </div>
            ...
            <div className="text-[10px] text-muted-foreground text-center pt-2 border-t border-border/40">
              Median recency {Math.round(report.dna.recencyMedian)}d · Median
              visits {Math.round(report.dna.frequencyMedian)} · Median spend{" "}
              {formatCurrency(report.dna.monetaryMedian)}
            </div>
          </TabsContent>
        </Tabs>
      </CardContent>
