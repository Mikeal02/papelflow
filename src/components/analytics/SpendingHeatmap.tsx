...
export const SpendingHeatmap = () => {
  ...
  const {
    ...
    const weekdays = heatmapData.filter((d) => {
      const dow = getDay(parseISO(d.day));
      return dow >= 1 && dow <= 5;
    });
    const weekends = heatmapData.filter((d) => {
      const dow = getDay(parseISO(d.day));
      return dow === 0 || dow === 6;
    });
    const wdAvg =
      weekdays.length > 0
        ? weekdays.reduce((s, d) => s + d.value, 0) / weekdays.length
        : 0;
    const weAvg =
      weekends.length > 0
        ? weekends.reduce((s, d) => s + d.value, 0) / weekends.length
        : 0;
  ...
  const selectedDayData = selectedDay
    ? heatmapData.find((d) => d.day === selectedDay)
    : null;
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-[3px]">
            ...
            {dayLabels.map((day, i) => (
              <div
                key={i}
                className="flex-1 text-center text-[9px] text-muted-foreground/70 font-bold uppercase tracking-wider"
              >
                {day}
              </div>
            ))}
            ...
            {weeks.map((week, weekIndex) => (
              <div key={weekIndex} className="flex gap-[3px]">
                <div className="w-7 flex-shrink-0 text-[9px] text-muted-foreground/50 flex items-center justify-end pr-1.5 font-semibold">
                  {weekIndex === 0 ||
                  weekIndex === Math.floor(weeks.length / 2) ||
                ...
                {Array.from({ length: 7 }, (_, dayIndex) => {
                  const day = week.find(
                    (d) => getDay(parseISO(d.day)) === dayIndex,
                  );
                  ...
                  const isPeak =
                    day && peakDay && day.day === peakDay.day && day.value > 0;
                    ...
                    <div key={dayIndex} className="flex-1 relative">
                      <motion.div
                        ...
                        className={cn(
                          ...
                          isHovered &&
                            "ring-2 ring-primary/60 scale-[1.3] z-10",
                        ...
                        onClick={() =>
                          day &&
                          setSelectedDay(
                            day.day === selectedDay ? null : day.day,
                          )
                        }
                      />
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs font-semibold">
                        {format(
                          parseISO(selectedDayData.day),
                          "EEEE, MMMM d, yyyy",
                        )}
                      </p>
                      <p className="text-[10px] text-muted-foreground mt-0.5">
                        {selectedDayData.transactions} transaction
                        {selectedDayData.transactions !== 1 ? "s" : ""}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-lg font-bold text-primary tabular-nums">
                        {formatCurrency(selectedDayData.value)}
                      </p>
                      <p className="text-[10px] text-muted-foreground">
                        {selectedDayData.value > avgDaily
                          ? `${((selectedDayData.value / avgDaily - 1) * 100).toFixed(0)}% above avg`
                          : `${((1 - selectedDayData.value / avgDaily) * 100).toFixed(0)}% below avg`}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {peakDay && (
              <p className="text-[9px] text-muted-foreground">
                ...
                <span className="font-semibold text-foreground">
                  {format(parseISO(peakDay.day), "MMM d")}
                </span>
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-4 gap-2 pt-3 border-t border-border/20">
            {[
              ...
              >
                <stat.icon
                  className={cn("h-3.5 w-3.5 mx-auto mb-1", stat.color)}
                />
                <p className="text-sm font-bold tabular-nums leading-none truncate">
                  {stat.value}
                </p>
