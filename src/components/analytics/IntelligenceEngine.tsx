...
export const IntelligenceEngine = () => {
  ...
  const [activeExpl, setActiveExpl] = useState<AlgorithmExplanation | null>(
    null,
  );
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  5 algorithms · {durationMs}ms compute · click any tile for
                  full trace
                </p>
              </div>
            </div>
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          ...
          <div className="stat-card !p-5">
            <PanelHeader
              ...
              badge={health.trajectory}
              onWhy={() => openExpl("health")}
            />
                ...
                <div key={key}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs tabular-nums text-muted-foreground">
                      {Math.round(dim.score)}/100 · w{dim.weight.toFixed(2)}
                    </span>
                  </div>
          ...
          <div className="stat-card !p-5">
            <PanelHeader
              ...
              badge="MAD z-score"
              onWhy={() => openExpl("anomaly")}
            />
            {anomalies.length === 0 ? (
              <p className="text-xs text-muted-foreground py-6 text-center">
                No anomalies detected. Spending is consistent. ✨
              </p>
            ) : (
          ...
          <div className="stat-card !p-5">
            <PanelHeader
              icon={Repeat}
              title="Auto-detected Recurring"
              badge="Fuzzy + autocorr."
              onWhy={() => openExpl("recurring")}
            />
            {recurring.length === 0 ? (
              <p className="text-xs text-muted-foreground py-6 text-center">
                No recurring patterns yet — keep logging.
              </p>
            ) : (
              <div className="space-y-2 max-h-72 overflow-y-auto">
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        Next ~{format(parseISO(r.nextPredictedDate), "MMM d")} ·{" "}
                    ...
                    <div className="text-right ml-2 shrink-0">
                      ...
                      <div className="text-[9px] text-muted-foreground tabular-nums">
                        {formatCurrency(r.totalAnnualCost)}/yr
                      </div>
                    </div>
                  </div>
                ))}
...
const MetricTile = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn("text-base font-bold tabular-nums truncate", toneClass)}
      >
        {value}
      </div>
