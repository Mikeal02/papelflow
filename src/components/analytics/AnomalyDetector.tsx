...
import {
  ...
  Severity,
  DetectorKey,
} from "@/lib/intelligence/anomalyElite";
...
export const AnomalyDetector = () => {
  ...
  const report = useMemo(
    () => runEliteAnomalies(transactions as any),
    [transactions],
  );
  ...
  const counts = useMemo(() => {
    const c = { critical: 0, high: 0, medium: 0, low: 0 } as Record<
      Severity,
      number
    >;
      ...
      <Card className="stat-card">
        <CardContent className="py-10 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            5-detector ensemble found no statistically meaningful outliers
            across {report.globals.scanned} expenses.
          </p>
        </CardContent>
      </Card>
    );
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">
              5-detector vote · {report.globals.flagged} flagged ·{" "}
              {report.globals.scanned} scanned
            </p>
          </div>
          <div className="text-right">
            ...
            <div
              className={cn(
                ...
                report.globals.health >= 70
                  ? "text-income"
                  : report.globals.health >= 40
                    ? "text-warning"
                    : "text-expense",
              )}
            >
        ...
        <Tabs defaultValue="alerts">
          ...
          <TabsContent value="alerts" className="space-y-2 mt-3">
            {filter !== "all" && (
              <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
                ...
                <Badge className={severityChip[filter as Severity]}>
                  {filter}
                </Badge>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {a.votes.filter((v) => v.fired).length >= 3 && (
                              <Badge
                                variant="outline"
                                className="text-[9px] bg-expense/10 text-expense border-expense/30"
                              >
                                Consensus
                              </Badge>
                            )}
                          ...
                          <p className="text-[11px] text-muted-foreground mt-0.5 line-clamp-1">
                            ...
                            {format(parseISO(a.date), "MMM d")} · expected{" "}
                            {formatCurrency(a.baseline.expected)}
                          </p>
                        ...
                        >
                          <div className="grid grid-cols-5 gap-1.5">
                            {a.votes.map((v) => (
                              ...
                              >
                                <div className="text-[8px] uppercase text-muted-foreground truncate">
                                  {v.label.split(" ")[0]}
                                </div>
                          ...
                          <div className="grid grid-cols-3 gap-2 mt-3 text-[10px]">
                            <div className="rounded bg-muted/30 p-2">
                              ...
                              <div className="font-semibold tabular-nums">
                                {formatCurrency(a.baseline.median)}
                              </div>
                            </div>
                            <div className="rounded bg-muted/30 p-2">
                              <div className="text-muted-foreground">
                            ...
                            <div className="rounded bg-muted/30 p-2">
                              ...
                              <div className="font-semibold tabular-nums">
                                {formatCurrency(a.baseline.spread)}
                              </div>
                            </div>
                          </div>
                        </motion.div>
            ...
            <div className="grid grid-cols-3 gap-2 mt-3 text-[10px]">
              <div className="rounded bg-muted/30 p-2">
                <div className="text-muted-foreground flex items-center gap-1">
                  <Gauge className="h-3 w-3" /> FP budget
                </div>
                <div className="font-semibold tabular-nums">
                  {report.globals.falsePositiveBudget}
                </div>
              </div>
              <div className="rounded bg-muted/30 p-2">
                <div className="text-muted-foreground">Coverage</div>
                <div className="font-semibold tabular-nums">
                  {Math.round(report.globals.coverage * 100)}%
                </div>
              </div>
              <div className="rounded bg-muted/30 p-2">
                <div className="text-muted-foreground">Health</div>
                <div className="font-semibold tabular-nums">
                  {Math.round(report.globals.health)}/100
                </div>
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="timeline" className="mt-3">
            ...
            <p className="text-[10px] text-muted-foreground text-center mt-1">
              Holt-Winters expected vs actual · bars show anomaly density (last
              30d)
            </p>
          </TabsContent>
        </Tabs>
