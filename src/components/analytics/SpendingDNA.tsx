...
export function SpendingDNA() {
  ...
  const report = useMemo(
    () => analyzeSpendingDna(transactions as any[]),
    [transactions],
  );
  const markov = useMemo(
    () => buildMerchantMarkov(transactions as any[], 8),
    [transactions],
  );
  const drift = useMemo(
    () => analyzeDrift(transactions as any[]),
    [transactions],
  );
  const shadow = useMemo(() => simulateShadowGenome(report), [report]);
  const forecast = useMemo(
    () => forecast30DaySpend(transactions as any[]),
    [transactions],
  );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Behavioral fingerprint across 7 orthogonal axes ·{" "}
                {report.diagnostics.find((d) => d.label === "Expenses analyzed")
                  ?.value || 0}{" "}
                txns analyzed
              </p>
            </div>
        ...
        {report.genome.length === 0 ? (
          ...
          <Tabs defaultValue="genome" className="space-y-4">
                ...
                <div className="space-y-1.5">
                  {report.archetypeBlend.slice(0, 3).map((a, i) => (
                    <div key={a.name} className="flex items-center gap-3">
                      ...
                      />
                      <div className="w-10 text-xs font-mono text-right tabular-nums">
                ...
                <div className="grid grid-cols-7 gap-1">
                  {DAYS.map((d, i) => {
                    ...
                    return (
                      <div
                        key={d}
                        className="rounded-lg border border-border/30 bg-card/60 backdrop-blur-sm p-2 text-center"
                      >
                        ...
                        <div className="text-[10px] font-mono mt-1 tabular-nums">
                          {(v * 100).toFixed(0)}%
                        </div>
                      </div>
                    );
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 gap-2">
                  {report.triggers.map((t) => (
                    <div
                      key={t.name}
                      className="flex items-center gap-2 rounded-lg border border-border/30 bg-card/60 backdrop-blur-sm p-2.5"
                    >
                      <Badge
                        variant="secondary"
            ...
            <TabsContent value="predict" className="m-0 space-y-3">
              <p className="text-xs text-muted-foreground">
                Cadence-based predictions for your most regular merchants.
                Probability reflects regularity (lower CV → higher confidence).
              </p>
              {report.nextLikely.length === 0 && (
                <div className="text-center py-8 text-sm text-muted-foreground">
                  Not enough merchant history yet. Need ≥3 transactions per
                  merchant.
                </div>
              )}
                ...
                {report.nextLikely.map((n, i) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <Badge
                        variant={n.probability >= 0.7 ? "default" : "secondary"}
                        className="text-[10px]"
                      >
                    ...
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-mono font-medium tabular-nums">
                        ${n.expectedAmountLow.toFixed(0)}–$
                        {n.expectedAmountHigh.toFixed(0)}
                      </span>
                    </div>
                    <div className="text-[10px] text-muted-foreground mt-1.5 pt-1.5 border-t border-border/30">
                      {n.reasoning}
                    </div>
                  </motion.div>
                ))}
            ...
            <TabsContent value="evolve" className="m-0 space-y-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-muted-foreground">
                    <GitBranch className="h-3 w-3" /> Behavioral drift (first vs
                    second half)
                  </div>
                ...
                {drift.axisDrift.length === 0 ? (
                  <div className="text-xs text-muted-foreground text-center py-4">
                    Need ≥20 transactions to measure drift.
                  </div>
                ) : (
                    ...
                    {drift.axisDrift.map((d) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 relative h-1.5 rounded-full bg-muted/40 overflow-hidden">
                            ...
                            <motion.div
                              ...
                              style={{
                                left: positive
                                  ? "50%"
                                  : `${50 - Math.min(50, Math.abs(d.delta) / 2)}%`,
                          ...
                          <div
                            className={cn(
                              "w-12 text-right font-mono tabular-nums font-semibold text-[11px]",
                              flat
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-muted-foreground">
                      <Sparkles className="h-3 w-3" /> Shadow genome (if recs
                      applied)
                    </div>
                    <Badge
                      variant="secondary"
                      className="text-[10px] text-income"
                    >
                      +{shadow.healthGain.toFixed(1)} health · ~$
                      {shadow.monthlySavings.toFixed(0)}/mo
                    </Badge>
                  </div>
                  <div className="h-56">
                    <ResponsiveContainer width="100%" height="100%">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-muted-foreground">
                      <Telescope className="h-3 w-3" /> 30-day spend forecast
                      (block bootstrap, 800 sims)
                    </div>
                  </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-muted-foreground">
                    <Network className="h-3 w-3" /> Merchant Markov chain ·
                    entropy {markov.entropyBits.toFixed(2)} bits
                  </div>
                  <Badge variant="secondary" className="text-[10px]">
                    {markov.states.length} states
                  </Badge>
                </div>
                {markov.states.length === 0 ? (
                  <div className="text-xs text-muted-foreground text-center py-4">
                    Need ≥5 transactions to build the chain.
                  </div>
                ) : (
                    ...
                    <div className="rounded-xl border border-border/30 bg-card/60 backdrop-blur-sm p-3">
                      ...
                      <div className="space-y-1.5">
                        {markov.predictedNext.map((p) => (
                          ...
                          >
                            <div className="w-28 truncate font-medium">
                              {p.merchant}
                            </div>
                            <Progress
                ...
                {markov.stationary.length > 0 && (
                  <div className="mt-3">
                    <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1.5">
                      Long-run merchant share (stationary π)
                    </div>
                    <div className="h-28">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart
                          data={markov.stationary}
                          layout="vertical"
            ...
            <TabsContent value="optimize" className="m-0 space-y-3">
              {report.recommendations.length === 0 ? (
                <div className="text-center py-8 text-sm text-muted-foreground">
                  Your spending DNA looks balanced — no high-impact behavioral
                  changes recommended.
                </div>
              ) : (
                <div className="space-y-2.5">
