...
export const FinancialPulse = () => {
  ...
  const statusConfig = {
    ...
    critical: {
      color: "text-expense",
      bg: "bg-expense/10",
      glow: "hsl(var(--expense))",
    },
  };

  const config = statusConfig[financialPulse.status];
  const pulseSpeed =
    financialPulse.status === "critical"
      ? 0.5
      : financialPulse.status === "fair"
        ? 0.8
        : 1.2;
      ...
      <Card className="stat-card overflow-hidden">
        ...
        <CardContent className="relative z-10">
            ...
            <div className="flex-1 space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span
                  className={cn(
                    "text-sm font-bold",
                    financialPulse.budgetAdherence >= 80
                      ? "text-income"
                      : "text-warning",
                  )}
                >
          ...
          <div className="mt-4 h-12 relative overflow-hidden">
            ...
            <div className="absolute inset-y-0 right-0 w-1/3 bg-gradient-to-l from-card to-transparent" />
          </div>
          ...
          >
            ...
            <span className="text-muted-foreground ml-1">
              {financialPulse.status === "excellent" &&
                "- Keep up the amazing work!"}
              {financialPulse.status === "good" &&
