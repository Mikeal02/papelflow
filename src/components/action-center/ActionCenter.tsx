...
export function ActionCenter() {
  ...
  const { actions, counts, byCategory, totalMinutes, totalImpact, total } =
    useActionCenter();
  ...
  const [filter, setFilter] = useState<"all" | ActionSeverity | ActionCategory>(
    "all",
  );
  ...
  const [lastDismissed, setLastDismissed] = useState<PriorityAction | null>(
    null,
  );
  ...
  const handleResolve = async (a: PriorityAction) => {
    if (a.cta.intent === "create_subscription" && a.cta.payload) {
      ...
      toast({
        ...
        description: `Now tracking ${a.cta.payload.name}`,
      });
    } else if (a.cta.route) {
      resolveAction(a);
  ...
  const handleSnooze = (a: PriorityAction, hours = 24) => {
    ...
    toast({
      ...
      description: "Will reappear automatically.",
    });
  };
  ...
  const bulkDismiss = () => {
    visible
      ...
      .forEach((a) => dismissAction(a.id, a));
  ...
  const bulkSnooze = () => {
    visible
      .filter((a) => selected.has(a.id))
      .forEach((a) => snoozeAction(a.id, 24, a));
  ...
  const filterOptions: { id: FilterId; label: string; n?: number }[] = (
    ...
    ] as { id: FilterId; label: string; n?: number }[]
      ...
      >
        ...
        <div className="border-t border-border/40 px-4 py-2.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-[9px] text-muted-foreground">
            <kbd className="bg-muted/50 px-1.5 py-0.5 rounded border border-border/40">
              /
            </kbd>
            <span>search</span>
            <kbd className="bg-muted/50 px-1.5 py-0.5 rounded border border-border/40">
...
function Stat({
  ...
  return (
    <div className="elite-card p-2 text-center overflow-hidden">
      ...
      <div
        className={cn("text-base font-bold tabular-nums leading-none", tone)}
      >
        {prefix}
        {value}
        {suffix}
      </div>
      <div className="text-[9px] text-muted-foreground uppercase tracking-wide mt-1 truncate">
        {label}
      </div>
    </div>
  );
    ...
    return (
      <div className="text-center py-12 px-4 elite-card mesh-bg">
        ...
        <p className="text-[11px] text-muted-foreground">
          Resolved, dismissed and snoozed actions will appear here.
        </p>
      </div>
    );
  }
        ...
        >
          ...
          {e.impact !== undefined && Math.abs(e.impact) > 0 && (
            <span className="text-[10px] font-semibold tabular-nums">
              {formatCurrency(Math.abs(e.impact))}
            </span>
          )}
        </div>
      ))}
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        pinned &&
          "before:absolute before:left-0 before:top-2 before:bottom-2 before:w-[3px] before:rounded-r before:bg-primary before:z-10",
      )}
    >
      {/* priority bar */}
      ...
      <div className="p-3 pt-3.5">
        <div className="flex items-start gap-2.5">
          ...
          <div
            className={cn(
              "relative h-9 w-9 rounded-xl conic-ring bg-card flex items-center justify-center shrink-0",
            )}
          >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
              ...
              <Badge
                variant="outline"
                className="text-[9px] capitalize h-4 px-1"
              >
                {action.category}
              </Badge>
              ...
              <Tooltip>
                ...
                <TooltipContent side="left" className="text-[10px] p-2">
                  ...
                  <ScoreLine
                    label="Severity"
                    v={action.score.severity}
                    max={40}
                  />
                  <ScoreLine label="Impact" v={action.score.impact} max={25} />
                  <ScoreLine
                    label="Urgency"
                    v={action.score.urgency}
                    max={20}
                  />
                  <ScoreLine
                    label="Confidence"
                    v={action.score.confidence}
                    max={10}
                  />
            ...
            >
              <div className="mt-3 grid grid-cols-2 gap-2">
                <div className="p-2.5 rounded-lg bg-muted/30 border border-border/30 space-y-1">
                  <p className="text-[9px] text-muted-foreground uppercase tracking-wide font-semibold mb-1">
                    Evidence
                  </p>
                ...
                {action.tags.length > 0 && (
                  <div className="col-span-2 flex flex-wrap gap-1">
                    {action.tags.map((t) => (
                      <span
                        key={t}
                        className="text-[9px] px-1.5 py-0.5 rounded-full bg-muted/40 text-muted-foreground border border-border/30"
                      >
                        #{t}
                      </span>
                    ))}
        ...
        <div className="flex items-center gap-1 mt-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem
                onClick={() => onSnooze(action, 1)}
                className="text-[12px]"
              >
                1 hour
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => onSnooze(action, 24)}
                className="text-[12px]"
              >
                Tomorrow
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => onSnooze(action, 24 * 7)}
                className="text-[12px]"
              >
                Next week
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => onSnooze(action, 24 * 30)}
                className="text-[12px]"
              >
                Next month
              </DropdownMenuItem>
