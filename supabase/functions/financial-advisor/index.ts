      ...
      .map((m: any) => ({
        ...
        content:
          typeof m?.content === "string"
            ? m.content.slice(0, MAX_MESSAGE_LEN)
            : "",
      }))
    ...
    const financialContext =
      fc && typeof fc === "object"
        ? {
            ...
            topCategories: Array.isArray(fc.topCategories)
              ? fc.topCategories.slice(0, 10).map(String)
              : [],
            activeGoals: fc.activeGoals,
            budgetStatus:
              typeof fc.budgetStatus === "string"
                ? fc.budgetStatus.slice(0, 200)
                : undefined,
          }
        : null;
