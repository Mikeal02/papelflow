...
const DATA_URL_RE =
  /^data:image\/(png|jpe?g|webp|heic|heif);base64,[A-Za-z0-9+/=]+$/;
...
serve(async (req) => {
  ...
  if (typeof imageBase64 !== "string")
    return json({ error: "no_image_provided" }, 400);
  if (imageBase64.length > MAX_IMAGE_BYTES)
    return json({ error: "image_too_large" }, 413);
  if (!DATA_URL_RE.test(imageBase64))
        ...
        messages: [
          {
            ...
            content: [
              {
                type: "text",
                text: `Analyze this receipt image and extract information as JSON:
{
...
Return ONLY valid JSON, no other text.`,
  ...
  if (!response.ok) {
    if (response.status === 429)
      return json({ error: "Rate limit exceeded. Please try again." }, 429);
